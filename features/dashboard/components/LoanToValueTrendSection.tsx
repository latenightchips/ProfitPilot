...
/**
 * Loan-to-Value Trend Section — v1.10.0 Batch 1 ("Dashboard Trend
 * Parity"). A compact, read-only historical view of the same
 * "Loan-to-Value" metric `app/portfolio/PortfolioHistoryPanel.tsx`
 ...
 * `LiquidationBufferTrendSection.tsx` (v1.8.0 Batch 1), and
 * `AnnualizedInterestCostTrendSection.tsx` (v1.9.0 Batch 1) already
 ...
 * surfaced.
 *
 * **Presentation/read-layer only — a direct field read.** Every
 * plotted value is `entry.loanToValue` itself — the identical field
 * `PortfolioHistoryPanel.tsx`'s own `loanToValue` metric config already
 * reads (see that file's `PORTFOLIO_HISTORY_METRICS` table). No new
 * formula, no Engine involvement, no new Formula ID, no recomputation
 * from today's portfolio or market state.
 *
 * **Always a plain, required number — no null handling, no
 * protocol-version branching.** `loanToValue` is a required, non-nullable
 ...
 */
function loanToValueForEntry(entry: PersistedPortfolioHistoryEntry): number {
  return entry.loanToValue;
}

export function LoanToValueTrendSection({
  portfolioId,
  portfolioUpdatedAt,
}: {
  portfolioId: string;
  ...
  if (status === 'loading') {
    return (
      <div className="flex flex-col gap-2 rounded-md border border-border p-4">
        <h3 className="text-sm font-medium text-foreground">Loan-to-Value Trend</h3>
  ...
  if (entries.length === 0) {
    return (
      <div className="flex flex-col gap-2 rounded-md border border-border p-4">
        <h3 className="text-sm font-medium text-foreground">Loan-to-Value Trend</h3>
        <p className="text-xs text-muted-foreground">No Loan-to-Value history yet.</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col gap-2 rounded-md border border-border p-4">
        ...
        <p className="text-xs text-muted-foreground">
          ...
          {formatDateTime(entries[0].createdAt)}): {formatPercent(loanToValueForEntry(entries[0]))}.
        </p>
      </div>
    );
  }
  ...
  const chartData = [...entries].reverse().map((entry) => ({
    ...
    value: loanToValueForEntry(entry),
  }));
  const chartSummary = `Loan-to-Value trend: ${chartData
    .map((point) => `${point.timestamp} ${formatPercent(point.value)}`)
    .join(', ')}`;
