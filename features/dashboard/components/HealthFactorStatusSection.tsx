 ...
 *
 * Renders exactly the Display list this task names, including "Risk
 * classification" — now `status.riskCategory` (F-026, owner decision,
 * PROJECT_STATUS.md conflict #1 closed; see
 * `../types/healthFactorStatus.ts` for the full reasoning). Rendered
 * with a `title="F-026 — see docs/02_Formulas.md"` tooltip, the same
 * Formula-ID-tooltip pattern the Current Health Factor value already
 * uses. Omitted only when `status.riskCategory` is `null` (the
 * unreachable-in-practice classification-failure case). "Formula
 * reference in Developer Mode" is a `title` tooltip on the Current
 * Health Factor value, the same minimal-baseline pattern
 * `DashboardKpiGrid` (M5-006) already established — no Developer Mode
 * toggle exists anywhere in this codebase yet (M5-022's own, later,
 * still-unbuilt task).
 *
 ...
 * ever shows on mouse hover, never on keyboard focus. The new Risk
 * Category value follows the identical convention.
 */
    ...
    <div className="flex flex-col gap-3 rounded-md border border-border p-4">
      ...
      <div className="flex flex-wrap gap-x-8 gap-y-2">
        ...
        {status.riskCategory !== null && (
          <div title="F-026 — see docs/02_Formulas.md" tabIndex={0}>
            <div className="text-xs text-muted-foreground">Risk Category</div>
            <div className="text-base font-medium text-foreground">{status.riskCategory}</div>
          </div>
        )}
