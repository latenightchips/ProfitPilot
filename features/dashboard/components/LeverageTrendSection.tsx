...
import { formatDateTime, formatLeverage } from '../utils/format';

/**
 * Leverage Trend Section — v1.10.0 Batch 2 ("Dashboard Trend Parity").
 * A compact, read-only historical view of the same "Leverage" metric
 ...
 * set to a metric Portfolio History has offered since v1.3.0 but the
 * Dashboard never surfaced.
 *
 * **Presentation/read-layer only — a direct field read, exactly the
 * already-persisted semantics.** Every plotted value is
 * `entry.leverage` itself — the identical field
 * `PortfolioHistoryPanel.tsx`'s own `leverage` metric config already
 * reads (see that file's `PORTFOLIO_HISTORY_METRICS` table). No
 * derivation, no reinterpretation, no new formula, no Engine
 * involvement, no new Formula ID, no recomputation from today's
 * portfolio or market state.
 *
 * **Formatted with the Dashboard's own `formatLeverage`**, the same
 * formatter `LeverageSummarySection`'s own current-value panel already
 * uses (via `buildLeverageSummary.ts`) — this trend chart's values read
 * consistently with the panel it pairs with.
 *
 * **Always a plain, required number — no null handling, no
 * protocol-version branching.** `leverage` is a required, non-nullable
 * `number` on every persisted entry regardless of protocol version
 * (`services/persistence/types/models.ts`), so this component has no
 * "no risk"/"not available" branch to render and never reads
 ...
 */
function leverageForEntry(entry: PersistedPortfolioHistoryEntry): number {
  return entry.leverage;
}

export function LeverageTrendSection({
  portfolioId,
  portfolioUpdatedAt,
}: {
  portfolioId: string;
  ...
  if (status === 'loading') {
    return (
      <div className="flex flex-col gap-2 rounded-md border border-border p-4">
        <h3 className="text-sm font-medium text-foreground">Leverage Trend</h3>
  ...
  if (entries.length === 0) {
    return (
      <div className="flex flex-col gap-2 rounded-md border border-border p-4">
        <h3 className="text-sm font-medium text-foreground">Leverage Trend</h3>
        <p className="text-xs text-muted-foreground">No Leverage history yet.</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col gap-2 rounded-md border border-border p-4">
        ...
        <p className="text-xs text-muted-foreground">
          ...
          {formatDateTime(entries[0].createdAt)}): {formatLeverage(leverageForEntry(entries[0]))}.
        </p>
      </div>
    );
  }
  ...
  const chartData = [...entries].reverse().map((entry) => ({
    ...
    value: leverageForEntry(entry),
  }));
  const chartSummary = `Leverage trend: ${chartData
    .map((point) => `${point.timestamp} ${formatLeverage(point.value)}`)
    .join(', ')}`;
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
            ...
            <YAxis
              width={40}
              tick={{ fontSize: 10 }}
              tickFormatter={(value: number) => formatLeverage(value)}
            />
            <Line
              type="monotone"
              dataKey="value"
