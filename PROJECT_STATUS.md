...
### Batch 13 — Simulation Warnings (M6-014)

**Repository re-sync required before this batch began, again — this
time using the hard-reset procedure directly, per instruction.** `git
ls-remote origin main` (run first, not `git fetch`'s own summary)
showed the real tip was `6b66f6e` — "feat(ui): implement simulation
assumptions panel (M6-013)." The local checkout had again gone stale at
an earlier commit, the same recurring behavior Batches 11–12 already
documented. `git checkout -B claude/profitpilot-repo-review-nty3yy
origin/main` reset the local branch directly to that verified tip;
`git status --short` and `git diff --stat HEAD origin/main` both came
back empty, confirming a clean, correctly-based tree before any
implementation began.

**Dependencies satisfied**: M6-009 (Scenario Summary) is synchronized
to GitHub.

**Only 2 of the 6 documented warning cases are built — the other 4
remain blocked or structurally unreachable, re-derived fresh for
Simulation rather than assumed identical to Dashboard's own,
differently-scoped M5-010 precedent:**

- **"Near liquidation"** — no documented proximity threshold exists
  distinct from Conflict #1's own disputed Health Factor risk bands,
  re-confirmed by a fresh grep across `01_PRD.md`/`02_Formulas.md`/
  `04_BUILD_GUIDE.md` before this batch began (not assumed from
  Dashboard's own prior finding). The only "near liquidation"-adjacent
  check anywhere in this codebase, `LIQUIDATION_PROXIMITY`
  (`engine/loop/validateLoopStrategySafety.ts`), checks
  `healthFactor <= 1.0` — the liquidation boundary itself, not a
  proximity buffer before it — and is scoped to Loop Strategy inputs,
  not reusable generically here.
- **"Invalid assumptions"** — structurally unreachable:
  `validateScenarioBuilderInput` (M6-004) already rejects every invalid
  field before `setCurrentScenario`/`runSimulation` is ever called, so
  a stored `currentScenario`/`currentResult` can never hold invalid
  assumptions — the same "validation gate makes the failure state
  unreachable" finding `features/dashboard/types/riskWarnings.ts`'s own
  "Invalid protocol parameters" case already established for
  `Portfolio`.
- **"High leverage"** — no numeric leverage threshold exists anywhere
  in the specification (confirmed by grep before this batch began) —
  only the unquantified principle "Never encourage excessive leverage."
- **"High borrowing cost"** — the same class of gap
  `riskWarnings.ts`'s own "High interest burden" case and conflict #29
  already found: no "acceptable" interest-cost threshold is documented
  anywhere.

**The 2 built cases reuse only already-real, already-documented values
— no new threshold was invented for either:**

- **"Unsafe Health Factor"** reuses `Portfolio.settings.safetyTargets
?.targetHealthFactor` — the user's own real, already-configured
  per-portfolio value (`buildHealthFactorStatus.ts`'s own field,
  Milestone 5) — and flags when the _simulated_ Health Factor falls
  below it. The exact same "distance from the user's own target, not an
  invented absolute risk band" pattern `riskWarnings.ts`'s own "Health
  Factor below configured target" case already established, applied
  here to a simulated value instead of the live portfolio's current
  one. Avoids Conflict #1 entirely, the same way that precedent does.
  Fires for both `currentResult` (price/interest scenarios) and
  `portfolioActionPreview` (portfolio actions) — `buildSimulationWarnings`
  takes a single `simulatedHealthFactor: number | null` parameter so it
  stays result-type-agnostic; the component computes which value is
  active.
- **"Stale prices"** reuses `normalizeMarketQuote`
  (`services/market/quote.ts`, M3-007, already public) against
  `Portfolio.marketUpdatedAt` — the real, documented "stale after 5
  minutes" rule from `04_BUILD_GUIDE.md`'s own "PRICE FRESHNESS"
  section, not an invented threshold. Reflects the _baseline
  portfolio's_ own price data quality, since a price/interest
  scenario's own overridden price has no independent freshness concept
  of its own.

**`SimulationWarnings.tsx` is the first Simulation component to take
`Portfolio` (`@/types/portfolio`), not `ApplicationPortfolio`.** Both
`targetHealthFactor` and `marketUpdatedAt` are fields `Portfolio` adds
on top of `ApplicationPortfolio` (M4-001) — every other Simulation
component simply never needed them. `app/simulation/page.tsx` already
holds the full `Portfolio` record (`record.portfolio`), so no new data
plumbing was needed, only a narrower prop type on this one component.

**Zero `engine/`/`services/`/`types/`/`stores/` changes — the cleanest
architectural footprint of any batch so far this milestone.** Every
value this batch needed (`Portfolio.settings.safetyTargets
.targetHealthFactor`, `Portfolio.marketUpdatedAt`, `normalizeMarketQuote`)
already existed and was already exported; this batch only added new
`features/simulation/` composition, no Store change at all — unlike
Batches 11 and 12, which each needed a new Store field.

**Field names deliberately follow M6-014's own literal DoD wording,
not Dashboard's M5-010 field names.** `SimulationWarning` uses
`reason`/`potentialImpact` ("Warnings explain both the cause and
potential impact") rather than reusing `RiskWarning`'s own
`reason`/`recommendedAction` (M5-010's differently-worded DoD, "reason
and recommended next action") — same shape, deliberately different
field name where the governing DoD text itself differs.

**A zero-warnings result renders positive confirmation text, not
nothing** — the same always-visible-section convention every other
Simulation component already follows, unlike `RiskWarningBanner.tsx`
(Dashboard, a floating banner that renders `null` on zero warnings).

**Manual browser verification used real UI interaction only.** A real
portfolio was created, a Target Health Factor of `5` was set on the
real Portfolio page's own Safety Target Settings field, then Simulation
was reached via in-app `<Link>` navigation. Screenshotted: (1) the
empty state before any simulation runs; (2) dropping BTC Price to
$12,000 produced a real `role="alert"` warning reading "The simulated
Health Factor (0.96) is below your configured target (5)" with its own
potential-impact text, alongside the documented "not checked" footnote
for the 4 blocked categories; (3) raising the price back up (with a
lower target of `2` in a second run) produced "No warnings for this
simulation." A stray second `[role="alert"]` element the verification
script initially flagged was investigated and confirmed to be Next.js's
own built-in `__next-route-announcer__` accessibility element, entirely
unrelated to this component — not a bug.

**`SimulationWarnings.tsx` line 47 is intentionally left uncovered —
genuinely unreachable, not an untested path.** The
`currentResult?.scenario.healthFactor ?? portfolioActionPreview?.after
.healthFactor` chain's "both null" branch can never execute: the
component's own early-return guard (`if (currentResult === null &&
portfolioActionPreview === null) return ...`) already ensures at least
one is non-null before this line runs. The same "unreachable given
valid inputs, kept for defense in depth" pattern already accepted for
`ScenarioSummary.tsx`'s own uncovered lines (Batch 8) — left uncovered
...
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `pnpm typecheck`             | ✅ Pass                                                                                                                                                                                                |
| `pnpm lint`                  | ✅ Pass (one `simple-import-sort` violation auto-fixed before final validation)                                                                                                                        |
| `pnpm format:check`          | ✅ Pass                                                                                                                                                                                                |
| `pnpm test` (Vitest)         | ✅ Pass, 1235/1235 (14 net new: `buildSimulationWarnings.test.ts`'s own 8 tests, `SimulationWarnings.test.tsx`'s own 4 tests, 2 `page.test.tsx` assertions)                                            |
| `pnpm test:coverage`         | ✅ 96.16% statements / 90.91% branches / 100% functions / 99.02% lines (project-wide); `SimulationWarnings.tsx` at 100% statements, one genuinely unreachable branch left uncovered (documented above) |
| `pnpm build`                 | ✅ Pass — `/simulation` grew from 106 kB to 107 kB (one new component, no new dependency)                                                                                                              |
| `pnpm test:e2e` (Playwright) | ✅ Pass, 35/35 (unchanged)                                                                                                                                                                             |

**Architecture audit**: `git diff --stat -- engine/ services/ types/
stores/` is completely empty — the first fully-empty architecture diff
since Batch 5 (M6-008), and the first time in this milestone that a new
Simulation feature needed zero new Store state at all. `git status
--porcelain` shows 3 modified files (2 source, 1 unit test) and 5 new
files (3 source, 2 unit test), no deletions.

**Traceability**: M6-014's Dependencies (M6-009) were satisfied as of
Batch 8; its Description (6 named warning categories) is addressed
item-by-item — 2 built with real, already-documented thresholds, 4
documented as blocked or structurally unreachable with fresh,
batch-specific evidence rather than assumed from Dashboard's own prior
finding; its DoD ("Warnings explain both the cause and potential
impact") is satisfied literally by the `reason`/`potentialImpact` field
pair on every built warning, verified by 12 new tests and direct manual
browser confirmation. M6-014 names no `Requirements` section — noted
explicitly, the same discipline Batches 11–12 already established.

---

### Batch 14 — Save Simulation (M6-015)

**Repository re-sync required before this batch began, again — hard
reset directly, per instruction.** `git ls-remote origin main` showed
the real tip was `918ada6` — "feat(ui): implement simulation warnings
(M6-014)." `git checkout -B claude/profitpilot-repo-review-nty3yy
origin/main` reset the local branch directly to that verified tip;
`git status --short` and `git diff --stat HEAD origin/main` both came
back empty before any implementation began.

**A second, mid-batch recovery was also required.** After this batch's
implementation, validation, and manual browser verification were
already complete and the user had approved the commit, the local
working tree unexpectedly reverted to an earlier commit (`M6-007`),
deleting this batch's entire uncommitted implementation from disk. A
fresh `git fetch` confirmed `origin/main` itself was unaffected (still
carrying the real Batch 13 tip, `918ada6`, with Batches 1–13 intact);
the local branch was re-synced to it and the implementation was redone
from scratch and re-validated in full — the second validation run
reproduced identical results (1243/1243 tests, identical coverage
percentages, identical bundle sizes) to the first, confirming the redo
was faithful.

**Dependencies satisfied**: M6-003 (Simulation Store) is synchronized
to GitHub.

**Closes a gap explicitly documented as a forward reference three times
already this milestone.** `stores/simulationStore.ts`'s own Batch 2
header comment, `ScenarioComparison.tsx`'s own Batch 9 header comment,
and this same PROJECT_STATUS.md's own Batch 9/10/11 write-ups all named
M6-015 as the task that would finally let a user actually save a
scenario. This batch is that task.

**All 4 named Include items addressed — "Timestamp" was already real
(`createdAt`, Batch 2); "Name," "Description," and "Portfolio
reference" are new.** `SavedSimulation` gained `name: string`,
`description: string | null`, and `portfolioId: string`.
`saveCurrentScenario` now takes a `SaveSimulationInput`
(`name`/`description?`/`portfolioId`) instead of no arguments — the
caller supplies `portfolioId` as a plain value, the same pattern
`runSimulation` already uses for `portfolio` itself, keeping the Store's
own documented independence from `usePortfolioStore` (M6-003's own DoD)
completely intact — no new import, no live binding.

**"Description" is optional, "Name" is required** — a direct,
literal reading of M6-015's own "Include: Name, Description" wording,
which names Description without the qualifying language "Name" reads
as requiring (the primary identifying field). Enforced by the same
inline-validation convention every other Simulation form field already
uses (`ScenarioBuilder.tsx`), not a new pattern.

**"Portfolio reference" is stored, not yet displayed or cross-checked**
— `04_BUILD_GUIDE.md`'s own "MULTI-PORTFOLIO SUPPORT" section lists
"Saved Simulations" as something each portfolio conceptually owns,
satisfied here by storing which portfolio was active at save time.
Detecting portfolio drift ("Display if portfolio has changed since
creation") is M6-016's own explicit Requirement, a separate, later
task — not pre-empted here.

**Only saves `currentScenario`/`currentResult` (price/interest
scenarios) — the same scope `saveCurrentScenario` has had since Batch
2, not newly narrowed.** Portfolio actions were never part of the
saved-scenario/comparison system (`ScenarioComparison.tsx`/
`ScenarioCharts.tsx` both already document this same structural gap);
extending `saveCurrentScenario` to accept a second result shape would
be inventing scope M6-015's own Dependencies (M6-003 only) do not ask
for.

**`ScenarioComparison.tsx` (M6-010, Batch 9) updated to use real
names** — both its checkbox list and its comparison table's own column
headers now show `saved.name` instead of a generic type-only label;
its own empty-state message and header comment (previously documenting
the M6-015 gap this batch closes) were updated to match. This is a
minimal, directly-enabled consequence of this batch's own new data,
not scope creep — the same "the data now exists, so the already-real
display naturally improves" pattern, not a new feature.

**Manual browser verification used real UI interaction only, saving
two real scenarios.** A real portfolio was created, two different BTC
Price scenarios were run and saved through the real form ("Bull Run
65k" at $65,000 and "Bear Case 40k" at $40,000, one with a real
Description), and Portfolio Comparison was confirmed to list both by
their real names with real timestamps; selecting both showed the real
names as the comparison table's own column headers — screenshotted.
`ScenarioCharts.tsx` (Batch 10) still shows generic "Price Scenario N"
labels, confirmed unmodified and out of this batch's own scope.

**`SaveSimulationForm.tsx`'s own null-return guard (`if (id === null)
return`) is intentionally left uncovered — a defensive check against a
render/submit race that real React reactivity makes practically
unreachable in a controlled test**, the same "kept for defense in
depth, not realistically triggerable" pattern already accepted for
`ScenarioSummary.tsx` (Batch 8) and `SimulationWarnings.tsx` (Batch
13).
...
| `pnpm lint`                  | ✅ Pass                                                                                                                                                                                                                                                                                        |
| `pnpm format:check`          | ✅ Pass                                                                                                                                                                                                                                                                                        |
| `pnpm test` (Vitest)         | ✅ Pass, 1243/1243 (8 net new: `SaveSimulationForm.test.tsx`'s own 6 tests, 1 new Store test for `saveCurrentScenario`'s own default-description case, 1 new `page.test.tsx` assertion; `ScenarioComparison.test.tsx`/`ScenarioCharts.test.tsx` call-site updates carried no net count change) |
| `pnpm test:coverage`         | ✅ 96.16% statements / 90.93% branches / 100% functions / 99.03% lines (project-wide); `SaveSimulationForm.tsx` at 100% statements, one genuinely-unreachable branch left uncovered (documented above)                                                                                         |
| `pnpm build`                 | ✅ Pass — `/simulation` grew from 107 kB to 107 kB (one new small form component, no new dependency)                                                                                                                                                                                           |
| `pnpm test:e2e` (Playwright) | ✅ Pass, 35/35 (unchanged)                                                                                                                                                                                                                                                                     |

**Architecture audit**: `git diff --stat -- engine/ services/ types/
stores/` shows only `stores/simulationStore.ts` (+38/-11 lines, the
`SavedSimulation` field additions and `saveCurrentScenario`'s new
input parameter) — no `engine/`, `services/`, or `types/` changes at
all, confirming no new Formula Engine or Service logic was written;
`portfolioId` is a plain caller-supplied value, not a new cross-Store
dependency. `git status --porcelain` shows 8 modified files (4 source,
4 unit test) and 2 new files (1 source, 1 unit test), no deletions.

**Traceability**: M6-015's Dependencies (M6-003) were satisfied as of
Batch 2; its Description ("Allow users to save scenarios") and all 4
named Include items are addressed — Timestamp was already real,
Name/Description/Portfolio reference are new and wired to a real,
inline-validated form; its DoD ("Saved simulations can be reopened
later") is satisfied by saved scenarios remaining browsable and
selectable in `ScenarioComparison.tsx` for the rest of the session
(Conflict B, in-memory-only until Milestone 8), verified by 8 new
tests and direct manual browser confirmation saving two real scenarios
end-to-end. M6-015 names no `Requirements` section — noted explicitly,
the same discipline Batches 11–13 already established.

---

### Batch 15 — Load Saved Simulation (M6-016)

**Repository re-sync required before this batch began, per
instruction.** `git ls-remote origin main` showed the real tip was
`17f7faa` — "feat(ui): implement save simulation (M6-015)."
...
--short` and `git diff --stat HEAD origin/main` both came back empty
before any implementation began.

**A mid-batch recovery was also required.** After this batch's
implementation, validation, and manual browser verification were
already complete and the user had approved the commit, the local
working tree unexpectedly reverted to an earlier commit (`M6-007`),
deleting this batch's entire uncommitted implementation from disk. A
fresh `git fetch`/`git ls-remote` confirmed `origin/main` itself was
unaffected (still carrying the real Batch 14 tip, `17f7faa`, with
Batches 1–14 intact); the local branch was re-synced to it and the
implementation was redone from scratch and re-validated in full — the
second validation run reproduced identical results (1250/1250 tests,
identical coverage percentages, identical bundle size) to the first,
confirming the redo was faithful.

**Dependencies satisfied**: M6-015 (Save Simulation) is synchronized to
GitHub as of Batch 14.

**The first Milestone 6 task since M6-011 to carry a formal
`Requirements` section — both items addressed literally, not folded
into a generic Description reading.** "Preserve original assumptions"
and "Display if portfolio has changed since creation" are M6-016's own
explicit Requirements, distinct from its one-line Description ("Open
previously saved simulations.").

**"Preserve original assumptions" is satisfied structurally: loading
restores the saved `scenario`/`result` directly, never recalling
`runSimulation`.** A new Store action, `loadSavedScenario(id)`, looks up
the matching `SavedSimulation` and sets `currentScenario`/`currentResult`
to its own already-computed `scenario`/`result` fields (computed once, at
save time, Batch 2) — the same "already computed, just read it back"
precedent `ScenarioComparison.tsx`'s own comparison table already
established for "without recalculation inside the UI" (M6-010). The
deliberate alternative — re-running `runSimulation` against the
_current_ live portfolio — was rejected: it would silently violate
reproducibility the moment the portfolio drifts, which is exactly the
failure mode this task's own second Requirement exists to surface, not
hide. `timelineProjection`/`lastMetadata`/`warnings` are all cleared on
load (none were captured at the original save time, so nothing stale is
left displayed).

**"Display if portfolio has changed since creation" reuses a real,
already-bumped field — `Portfolio.updatedAt` — never an invented
staleness heuristic.** `stores/portfolioStore.ts`'s own `update` action
(confirmed by direct reading, lines 325–390) sets `updatedAt: now`
unconditionally on every mutation, regardless of which specific field
changed. A new `portfolioUpdatedAt` snapshot field was added to
`SavedSimulation`/`SaveSimulationInput` this batch (not retroactively
added to Batch 14 — M6-015's own Include list never named it, and this
task's own distinct Requirement is what creates the need, the same
"each task adds exactly what its own scope requires" discipline already
used for `warnings`/`timelineProjection`/`lastMetadata`). The comparison
itself lives in `ScenarioComparison.tsx`'s own new `driftNotice` helper,
not the Store — keeping the Store's own documented independence from
`usePortfolioStore` (M6-003's own DoD, "Simulation state is completely
independent from portfolio state") fully intact; the Store only ever
stores the plain snapshot value, it never imports or compares against
`usePortfolioStore` itself.

**A saved scenario belonging to a different portfolio entirely is
flagged with a distinct message, a genuine edge case, not a
hypothetical.** Since `savedScenarios` lives in the Simulation Store and
is never cleared when the active portfolio is switched (M6-003's own
independence design, true since Batch 2), a saved scenario from another
portfolio can genuinely still be sitting in the list once `portfolioId`
exists to check against. `driftNotice` checks `portfolioId` first
("Saved against a different portfolio.") before falling through to the
same-portfolio-but-changed check ("Portfolio has changed since this was
saved.") — two distinct messages for two distinct situations, neither
fabricated.

**Loading does not resync `ScenarioBuilder.tsx`'s own local form
fields — a documented, known limitation, not an oversight.**
`ScenarioBuilder`'s `values` state has only ever written to the Store,
never read `currentScenario` back out (true since Batch 3); making it
do so would be a broader architecture change this task's own narrow DoD
("Historical simulations remain reproducible" — about the displayed
_result_, not the input form) does not ask for. The loaded scenario's
own numbers are correctly reproduced everywhere they are displayed
(Simulation Results, Assumptions); only the Scenario Builder's own input
fields keep showing whatever was last typed. Documented directly in
`ScenarioComparison.tsx`'s own header comment rather than left silent.

**"Load" was added to `ScenarioComparison.tsx` (M6-010, Batch 9)
itself, not a new component.** It is the only place a saved scenario is
ever reopened from, and it already owns the full `savedScenarios` list
and each entry's own render — no new source file was added this batch,
the first batch in this milestone not to add one.

**Manual browser verification used real UI interaction only, covering
both Requirements end-to-end.** A real portfolio was created; a Price
Scenario was run at BTC Price $65,000 and saved as "Bull Run 65k"; the
...
exact originally-computed values were restored, screenshotted. Drift
detection was then verified separately: immediately after saving, no
drift notice appeared (same portfolio, unchanged); a real portfolio
mutation was made through the actual Portfolio page form (Manual price
changed to $55,000, via the real "Preview Changes" → "Apply Changes"
flow, not a direct field edit — Apply stays disabled until Preview is
clicked first, discovered while writing the verification script);
navigating back to Simulation via real in-app link clicks (never
`page.goto()`, which would wipe the in-memory Store per Conflict B)
confirmed the drift notice "Portfolio has changed since this was
saved." now correctly rendered in destructive/red styling next to "Bull
Run 65k," while the loaded scenario's own displayed values remained
untouched — screenshotted as final confirmation.
...
| `pnpm lint`                  | ✅ Pass                                                                                                                                                           |
| `pnpm format:check`          | ✅ Pass                                                                                                                                                           |
| `pnpm test` (Vitest)         | ✅ Pass, 1250/1250 (7 net new: 3 new `simulationStore.test.ts` tests for `loadSavedScenario`, 4 new `ScenarioComparison.test.tsx` tests for Load/drift detection) |
| `pnpm test:coverage`         | ✅ 96.19% statements / 90.98% branches / 100% functions / 99.04% lines (project-wide)                                                                             |
| `pnpm build`                 | ✅ Pass — `/simulation` unchanged at 107 kB (one new Store action, one modified existing component, no new dependency)                                            |
| `pnpm test:e2e` (Playwright) | ✅ Pass, 35/35 (unchanged)                                                                                                                                        |

**Architecture audit**: `git diff --stat -- engine/ services/ types/
stores/` shows only `stores/simulationStore.ts` (the new
`portfolioUpdatedAt` field on `SavedSimulation`/`SaveSimulationInput`
and the new `loadSavedScenario` action) — no `engine/`, `services/`, or
`types/` changes at all, confirming no new Formula Engine or Service
logic was written; the drift comparison itself lives entirely in
`ScenarioComparison.tsx`, reusing the already-real `Portfolio.updatedAt`
field rather than adding a new cross-Store dependency. `git status
--porcelain` shows 8 modified files (3 source, 4 unit test, this
write-up), no new or deleted files.

**Traceability**: M6-016's Dependencies (M6-015) were satisfied as of
Batch 14; its Description ("Open previously saved simulations.") is
satisfied by the new "Load" button in `ScenarioComparison.tsx`; both
named Requirements are addressed — "Preserve original assumptions" by
`loadSavedScenario`'s direct restore (no recalculation), "Display if
portfolio has changed since creation" by the new `driftNotice`
comparison against real `Portfolio.updatedAt`; its DoD ("Historical
simulations remain reproducible") is satisfied and verified by 7 new
tests plus direct manual browser confirmation of an exact value
restoration after an intervening change. M6-016 is the first task since
M6-011 to name a formal `Requirements` section — noted explicitly, not
silently treated the same as Batches 12–14's Requirements-less tasks.

---

### Batch 16 — Duplicate Simulation (M6-017)

**Repository re-sync required before this batch began, per
instruction.** `git ls-remote origin main` showed the real tip was
`88e02b0` — "feat(ui): implement load saved simulation (M6-016)."
`git checkout -B claude/profitpilot-repo-review-nty3yy origin/main`
reset the local branch directly to that verified tip; `git status
...
deleting this batch's entire uncommitted implementation from disk. A
fresh `git fetch`/`git ls-remote` confirmed `origin/main` itself was
unaffected (still carrying the real Batch 15 tip, `88e02b0`, with
Batches 1–15 intact); the local branch was re-synced to it and the
implementation was redone from scratch and re-validated in full — the
second validation run reproduced identical results (1256/1256 tests,
identical coverage percentages, identical bundle size) to the first,
confirming the redo was faithful.
...
**M6-017 names no `Requirements` section of its own — only a
Description ("Allow users to duplicate a saved scenario for
experimentation") and a DoD ("Copies are fully independent").** Rather
than invent a shape for "duplicate" from nothing, this batch reused
`stores/portfolioStore.ts`'s own already-approved `duplicate` action
(M4-011, "Implement Portfolio Duplication") verbatim as precedent for
what "duplicate" already means in this exact codebase: a new identity
(`crypto.randomUUID()`), a fresh `createdAt`, and the name suffixed
with `" (Copy)"`. `scenario`/`result`/`description`/`portfolioId`/
`portfolioUpdatedAt` are all carried over unchanged — the duplicate
represents the same assumptions, ready to be loaded and further
experimented with via M6-016's own `loadSavedScenario`, not a blank
slate.

**"Copies are fully independent" is satisfied structurally, not just by
intent.** The new entry gets its own `id`, so `deleteSavedScenario`/
`toggleComparisonSelection` acting on one never touches the other, and
neither this Store nor any component ever mutates a `SavedSimulation`'s
own fields in place after creation — the copy and the original share no
mutable reference either has a way to corrupt. `duplicateSavedScenario`
returns the new record's real `id` (or `null` if the source `id` does
not match any saved scenario), the same `string | null` contract
`saveCurrentScenario` already uses for "creates a new saved record."

**"Duplicate" was added to `ScenarioComparison.tsx` (M6-010, Batch 9)
itself, not a new component** — the same placement reasoning Batch 15
already established for "Load": this is the only place a saved
scenario is ever rendered, so it owns every action that can be taken on
one. No new source file was added this batch, the second batch in a
row (after Batch 15) to extend an existing component rather than add
one.

**This batch's own mandatory manual browser verification found and
fixed one real, pre-existing bug, unrelated to M6-017's own scope — the
same "found-and-fixed-during-verification" precedent Batch 8 already
established.** While typing a BTC Price through intermediate keystroke
values (a deeply negative-equity price briefly active mid-keystroke),
the browser console logged a React "duplicate key" error:
`ScenarioSummary.tsx`'s warnings list (Batch 9) keyed each row by
`warning.code` alone, but a `ServiceWarning[]` can genuinely contain two
entries with the same code. Fixed by keying on
`` `${warning.code}-${index}` `` instead — no change to what is
displayed, only to React's own row identity — and covered by a new
regression test asserting two same-coded warnings both render as
distinct rows.

**Manual browser verification used real UI interaction only.** A real
portfolio was created; a Price Scenario was run at BTC Price $65,000
and saved as "Bull Run 65k"; clicking "Duplicate" was confirmed to add
a second, independent row — "Bull Run 65k (Copy)" — immediately, with
its own Load/Duplicate controls, while the original row remained
untouched. The active scenario was then changed to $30,000 (confirming
Simulation Results updated), and clicking "Load" on the _copy_
specifically was confirmed to independently restore the exact original
$65k-scenario values (Portfolio Value $80,000.00 → $110,000.00,
unchanged) — proving the copy is a fully independent, correctly-
reproducing record, not a shared reference to the original — screenshotted
at both stages.
...
| `pnpm lint`                  | ✅ Pass                                                                                                                                                                                                                              |
| `pnpm format:check`          | ✅ Pass                                                                                                                                                                                                                              |
| `pnpm test` (Vitest)         | ✅ Pass, 1256/1256 (6 net new: 3 `simulationStore.test.ts` tests for `duplicateSavedScenario`, 2 `ScenarioComparison.test.tsx` tests for the Duplicate button, 1 `ScenarioSummary.test.tsx` regression test for the warning-key fix) |
| `pnpm test:coverage`         | ✅ 96.21% statements / 91% branches / 100% functions / 99.04% lines (project-wide)                                                                                                                                                   |
| `pnpm build`                 | ✅ Pass — `/simulation` unchanged at 107 kB (one new Store action, two modified existing components, no new dependency)                                                                                                              |
| `pnpm test:e2e` (Playwright) | ✅ Pass, 35/35 (unchanged)                                                                                                                                                                                                           |

**Architecture audit**: `git diff --stat -- engine/ services/ types/
stores/` shows only `stores/simulationStore.ts` (the new
`duplicateSavedScenario` action) — no `engine/`, `services/`, or
`types/` changes at all, confirming no new Formula Engine or Service
logic was written; `duplicateSavedScenario` is pure list manipulation
inside the Store, reusing an already-established codebase convention
rather than adding new calculation logic. `git status --porcelain`
shows 7 modified files (3 source, 3 unit test, this write-up), no new
or deleted files.

**Traceability**: M6-017's Dependencies (M6-016) were satisfied as of
Batch 15; its Description ("Allow users to duplicate a saved scenario
for experimentation") is satisfied by the new "Duplicate" button in
`ScenarioComparison.tsx`; its DoD ("Copies are fully independent") is
satisfied and verified by 5 new tests (3 Store-level, 2
component-level) plus direct manual browser confirmation that the copy
survives and correctly reproduces its own values independently of the
original. M6-017 names no `Requirements` section — noted explicitly,
the same discipline Batches 11–13 already established for
Requirements-less tasks.

---

### Batch 17 — Delete Simulation (M6-018)

**Repository re-sync required before this batch began, per
instruction.** `git ls-remote origin main` showed the real tip was
`7d59a8d` — "feat(ui): implement duplicate simulation (M6-017)."
`git checkout -B claude/profitpilot-repo-review-nty3yy origin/main`
reset the local branch directly to that verified tip; `git status
--short` confirmed a clean working tree before any implementation
began.
...
deleting this batch's entire uncommitted implementation from disk. A
fresh `git fetch`/`git ls-remote` confirmed `origin/main` itself was
unaffected (still carrying the real Batch 16 tip, `7d59a8d`, with
Batches 1–16 intact); the local branch was re-synced to it and the
implementation was redone from scratch and re-validated in full — the
second validation run reproduced identical results (1261/1261 tests,
identical coverage percentages, identical bundle size) to the first,
confirming the redo was faithful.
...
**M6-018 names no `Requirements` section of its own — only a
Description ("Delete saved simulations with confirmation") and a DoD
("Deletion cannot occur accidentally").** `deleteSavedScenario`
(`stores/simulationStore.ts`) has been a real Store action since
Batch 2/M6-003, but no UI ever called it until this batch — the same
"action exists in the Store, waiting for its own dedicated later task"
pattern already true of `loadSavedScenario` (Batch 15) and
`duplicateSavedScenario` (Batch 16) before their own tasks arrived.
This batch's whole job was wiring it to a button, safely.

**"With confirmation"/"cannot occur accidentally" reuses
`app/portfolios/page.tsx`'s own already-approved inline, per-row
delete-confirmation pattern (M4-012, "Implement Portfolio Archive and
Delete") rather than inventing a second confirmation UX.** Clicking
"Delete" does not delete anything — it only sets a local
`confirmingDeleteId` and opens an inline panel naming the scenario
(`Delete &ldquo;{saved.name}&rdquo;?`) with an explicit "This
permanently removes this saved simulation. This cannot be undone."
warning. Only "Confirm Delete" inside that panel calls
`deleteSavedScenario`; "Cancel" (or simply not clicking Confirm)
leaves the record untouched. Only the confirm/cancel _shape_ of the
Portfolio page's own pattern was reused, not its "replacement
portfolio" branch — saved scenarios have no "active" concept a
deletion could leave dangling, so that entire additional step does not
apply here and was not built.

**Needed zero `engine/`/`services/`/`types/`/`stores/` changes at
all — the first batch since Batch 13 with a fully clean architecture
diff.** `deleteSavedScenario`'s own guard logic (removing the record
and clearing it from `comparisonSelection`) was already complete and
already tested since Batch 2; this batch only added a button, local
`useState`, and a confirmation panel to `ScenarioComparison.tsx`.

**Manual browser verification used real UI interaction only, covering
all three states of the confirmation flow.** A real portfolio was
created, a Price Scenario was run and saved as "Bull Run 65k."
Clicking "Delete" once was confirmed to leave the record in place
while opening the inline confirm panel (screenshotted). Clicking
"Cancel" was confirmed to close the panel with the record still
present. Clicking "Delete" again followed by "Confirm Delete" was
confirmed to actually remove the record, with "Portfolio Comparison"
correctly reverting to "No scenarios saved yet." (screenshotted).
...
| `pnpm lint`                  | ✅ Pass                                                                                                                                                                                   |
| `pnpm format:check`          | ✅ Pass                                                                                                                                                                                   |
| `pnpm test` (Vitest)         | ✅ Pass, 1261/1261 (5 net new: `ScenarioComparison.test.tsx` tests covering bare-click-does-not-delete, Confirm Delete, Cancel, multi-row independence, and comparison-selection cleanup) |
| `pnpm test:coverage`         | ✅ 96.22% statements / 91.01% branches / 100% functions / 99.05% lines (project-wide)                                                                                                     |
| `pnpm build`                 | ✅ Pass — `/simulation` unchanged at 107 kB (one modified existing component, no new dependency)                                                                                          |
| `pnpm test:e2e` (Playwright) | ✅ Pass, 35/35 (unchanged)                                                                                                                                                                |

**Architecture audit**: `git diff --stat -- engine/ services/ types/
stores/` shows no output at all — zero files changed under any of
those directories, confirming no new Formula Engine, Service, or Store
logic was written; every value and action this batch needed already
existed. `git status --porcelain` shows 2 modified files (1 source, 1
unit test) plus this write-up, no new or deleted files.

**Traceability**: M6-018's Dependencies (M6-016) were satisfied as of
Batch 15; its Description ("Delete saved simulations with
confirmation") is satisfied by the new "Delete" button plus inline
confirm panel in `ScenarioComparison.tsx`; its DoD ("Deletion cannot
occur accidentally") is satisfied and verified by 5 new tests plus
direct manual browser confirmation that a bare click and a Cancel both
leave the record untouched, and only "Confirm Delete" removes it.
M6-018 names no `Requirements` section — noted explicitly, the same
discipline Batches 11–13/16 already established for Requirements-less
tasks.

---

### Batch 18 — Export Simulation (M6-019)

**Repository re-sync required before this batch began, per
instruction.** `git ls-remote origin main` showed the real tip was
`2ed0ab7` — "feat(ui): implement delete simulation (M6-018)."
`git checkout -B claude/profitpilot-repo-review-nty3yy origin/main`
reset the local branch directly to that verified tip; `git status
--short` confirmed a clean working tree before any implementation
began.
...
already complete and the user had approved the commit, the local
working tree unexpectedly reverted to an earlier commit (`M6-007`),
deleting this batch's entire uncommitted implementation from disk,
including its two new, previously-untracked source files. A fresh
`git fetch`/`git ls-remote` confirmed `origin/main` itself was
unaffected (still carrying the real Batch 17 tip, `2ed0ab7`, with
Batches 1–17 intact); the local branch was re-synced to it and the
implementation was redone from scratch and re-validated in full — the
second validation run reproduced identical results (1283/1283 tests,
identical coverage percentages, identical bundle size) to the first,
confirming the redo was faithful.
...
**Before writing any code, confirmed `services/export/` (M3-001's own
scaffold) is a genuinely different concept, not a duplicate or a
pre-empted task.** That directory's own header comment names its
eventual contents explicitly — `ExportService.ts`/`JsonExporter.ts`/
`CsvExporter.ts`/`PdfExporter.ts` — and states its implementation is
"a separate, dependent, later-milestone task." Grepping `06_TASKS.md`
confirmed exactly which: Milestone 8 owns its own dedicated "Create
Export Service," "Implement Full JSON Export," "Implement
Single-Record JSON Export," "Implement CSV Export," and "Implement
Export File Naming" tasks — a full-application backup/restore system
for every persisted record, unrelated to M6-019's own narrower
"export one Simulation's own already-computed result." Building this
inside `services/export/` now would have been inventing Milestone 8's
own scope ahead of time. `utils/portfolioRecoveryExport.ts` (M4-017)
already drew the exact same distinction for its own narrower
portfolio-recovery export — this batch's export logic lives in the
Simulation feature instead (`features/simulation/utils/
exportSimulation.ts` + a new `ExportSimulation.tsx`), reusing that
same file's own already-approved Blob + temporary-anchor download
pattern verbatim.

**"Formats: JSON, CSV, Future PDF"** — only JSON/CSV were built; PDF
is 06_TASKS.md's own explicit deferral marker, the same word
`01_PRD.md`'s "BACKUP & RECOVERY" section uses for "Cloud backup"/
"Encrypted snapshots," neither ever built. No PDF library was added.

**"Include: Inputs, Outputs, Assumptions, Timestamp, Formula version"
mapped directly onto already-real fields, with one genuine gap that
required a small, justified Store extension.** Inputs → the exact
`SimulationScenario`; Outputs → `result.baseline`/`result.scenario`/
`result.comparison` (already-computed, never recalculated, matching
`ScenarioComparison.tsx`'s own "without recalculation" precedent);
Assumptions → price/rate assumptions (`result.assumptions`, the same
field `SimulationAssumptions.tsx` already reads) plus Protocol
Parameters, read from the live `portfolio` prop (the exact same source
and reasoning that component already established, since whether the
live portfolio has drifted from a saved simulation's own original
values is already surfaced by `ScenarioComparison.tsx`'s own
`driftNotice` before a user ever reaches Load — Export does not
duplicate that concern). "Timestamp"/"Formula version" had no honest
source for a _saved_ record — `lastMetadata` is ephemeral, and
`SavedSimulation` never captured it — so `SavedSimulation` gained a
`metadata: ServiceMetadata | null` snapshot, captured internally from
the Store's own `lastMetadata` inside `saveCurrentScenario` (not
caller-supplied, since the Store already owns this value), the same
"each task adds exactly what its own scope requires" discipline
`portfolioUpdatedAt` (Batch 15) already established.

**`loadSavedScenario` was corrected to restore `lastMetadata` from that
snapshot instead of clearing it to `null` — a deliberate, documented
correction to Batch 15's own prior behavior, not a silent
regression.** Batch 15 cleared it because nothing was captured at save
time yet; now that `metadata` is real, clearing it on Load would
misrepresent a loaded simulation as having no known Formula version,
when it verifiably does. **This also fixed a latent, previously-
unreachable gap in `SimulationAssumptions.tsx`'s own "Formula Version"
row (M6-013, Batch 12), which silently disappeared after every Load
before this batch** — found during this batch's own mandatory manual
browser verification, confirmed by direct testing rather than merely
assumed, the same "found-and-fixed-during-verification" precedent
Batch 8/16 already established. A regression test was added to
`SimulationAssumptions.test.tsx` covering exactly this.

**Export lives next to "Simulation Results," not inside
`ScenarioComparison.tsx`'s saved-row list.** `03_UI.md`'s own "EXPORT"
section names "Scenario Summary" as what gets exported — the same
result `ScenarioSummary.tsx` (M6-009) already renders for whichever
scenario is currently active. This also explains M6-019's own
Dependency on M6-016 without requiring a second, saved-row-specific
export implementation: exporting an _old_ saved simulation means
clicking "Load" first (already real), then using these same Export
controls — and because `loadSavedScenario` now restores the original
`metadata`, that round trip reports the _original_ Formula
version/timestamp, not a fabricated current one, verified directly by
a dedicated round-trip test and by manual browser verification.
...
saved-scenario/comparison system this task's own Dependency builds on.

**`schemaVersion` is included in every export**, matching `01_PRD.md`'s
"BACKUP & RECOVERY" section ("Every export should include schema
versioning") — a cross-cutting rule, not specific to
`utils/portfolioRecoveryExport.ts`'s own recovery copy, which already
established the same field for the same reason.

**No formal file-naming convention was invented** —
`06_TASKS.md` names "Implement Export File Naming" as Milestone 8's
own separate, later task. Filenames here
(`simulation-export-<scenario-type>.<ext>`) are simple and functional,
not a designed scheme.

**CSV shape is a documented design decision, not a specification
conflict.** Neither `06_TASKS.md` nor `03_UI.md` specifies a CSV
layout; a flat two-column `Field,Value` table was chosen since CSV
cannot represent the JSON export's nested structure, with standard
quoting/escaping applied to any value containing a comma, quote, or
newline (a dedicated `csvEscape` unit, exported and directly tested).

**Manual browser verification used real UI interaction only, covering
both formats and the Load round trip.** A real portfolio was created;
before any scenario ran, the Export section correctly showed its own
placeholder message, not buttons. A Price Scenario was run at BTC
Price $65,000; clicking "Export JSON" and "Export CSV" both triggered
real browser downloads, captured and inspected directly — the JSON
contained the correct `schemaVersion`, `inputs.type`, `outputs.scenario.equity`
($110,000), real `protocolParameters`, and non-null `timestamp`/
`formulaVersion`; the CSV began with `Field,Value` and contained the
expected rows. The scenario was then saved, the active scenario
changed to $30,000, and the original was Loaded back — the "Formula
Version" row was confirmed still visible in Simulation Assumptions
(the fix, visually verified), screenshotted at both stages.
...
| `pnpm lint`                  | ✅ Pass                                                                                                                                                                                                                       |
| `pnpm format:check`          | ✅ Pass                                                                                                                                                                                                                       |
| `pnpm test` (Vitest)         | ✅ Pass, 1283/1283 (22 net new: `exportSimulation.test.ts` and `ExportSimulation.test.tsx`, both new files, plus `simulationStore.test.ts`/`SimulationAssumptions.test.tsx` additions for the `metadata` capture/restore fix) |
| `pnpm test:coverage`         | ✅ 96.28% statements / 91.13% branches / 100% functions / 99.07% lines (project-wide); `features/simulation/utils/exportSimulation.ts` at 100% statements/lines                                                               |
| `pnpm build`                 | ✅ Pass — `/simulation` grew from 107 kB to 109 kB (one new component, one new utils file, no new dependency)                                                                                                                 |
| `pnpm test:e2e` (Playwright) | ✅ Pass, 35/35 (unchanged)                                                                                                                                                                                                    |

**Architecture audit**: `git diff --stat -- engine/ services/ types/
stores/` shows only `stores/simulationStore.ts` (the new `metadata`
field and its capture/restore) — no `engine/`, `services/`, or
`types/` changes at all, confirming no new Formula Engine or Service
logic was written, and confirming `services/export/` itself was
correctly left untouched (Milestone 8's own reserved scope). `git
status --porcelain` shows 5 modified files and 4 new files (2 source,
2 unit test), no deletions.

**Traceability**: M6-019's Dependencies (M6-016) were satisfied as of
Batch 15; its Description ("Export simulation results") and named
Formats (JSON, CSV built; PDF explicitly deferred as "Future") are
satisfied by `ExportSimulation.tsx`'s two buttons; all 5 named Include
items (Inputs, Outputs, Assumptions, Timestamp, Formula version) are
addressed and mapped explicitly above; its DoD ("Exported simulations
are reproducible") is satisfied by exporting already-computed values
verbatim, never recalculating, and by the Load round trip correctly
preserving original metadata rather than substituting current
(possibly different) values — verified by 22 new tests plus direct
manual browser confirmation of both export formats and the round trip.
...
### Batch 20 — Responsive Workspace (M6-021)

**Repository re-sync required before this batch began, per
instruction.** `git ls-remote origin main` showed the real tip was
`9a72976` — "feat(ui): implement simulation history (M6-020)."
`git checkout -B claude/profitpilot-repo-review-nty3yy origin/main`
reset the local branch directly to that verified tip; `git status
--short` confirmed a clean working tree before any implementation
began. No mid-batch working-tree reset occurred this batch either —
the second consecutive batch to complete without one.

**Dependencies satisfied**: M6-020 (Simulation History) is
synchronized to GitHub as of Batch 19.

**M6-021's own text is short and un-mocked, unlike its Dashboard
counterpart M5-023.** `06_TASKS.md` gives M6-021 no `Requirements`
section and no explicit Desktop/Tablet/Mobile column breakdown — only
a one-line Description ("Optimize Simulation Workspace for desktop,
tablet and mobile") and a one-line DoD ("Simulation tools remain
usable on supported screen sizes"). Unlike M5-023, `03_UI.md` names no
dedicated layout mockup for this either. Given this, `06_TASKS.md`'s
own literal wording was treated as the sole scope, the same precedent
every other un-mocked Milestone 6 task has already established —
nothing beyond "verify it's usable at supported widths" was invented.

**Investigated by rendering the real, compiled app at real viewport
widths via Playwright — the same method Batch 12 (M5-023) established
for the Dashboard — and found zero real bugs, a genuine negative
result, not a shortcut.** A portfolio with a deliberately long name
(58+ characters, to stress `AppHeader`'s own portfolio switcher) was
created; three scenarios were saved (one an active interest scenario,
to populate Scenario Charts and Scenario Timeline); all three were
selected in Scenario Comparison to render its own comparison table.
`document.documentElement.scrollWidth <= clientWidth` was checked at
375px, 768px (the sidebar-appears breakpoint, the same width Batch 12
found its own real bugs at), and 1280px — **no horizontal page overflow
was found at any width**, unlike Batch 12's own Dashboard audit, which
found two real bugs at this exact stage of investigation.

**Why no bugs were found: every layout mechanism this route depends on
was already built responsively by the batch that introduced it, not
retrofitted now.** `app/simulation/page.tsx`'s own `flex-col
lg:flex-row` sidebar/content split has been real since M6-001 (Batch
1); `ScenarioComparison.tsx`'s own `overflow-x-auto` comparison-table
container has been real since M6-010 (Batch 9); `ScenarioCharts.tsx`/
`ScenarioTimeline.tsx`'s own recharts `ResponsiveContainer` usage has
been real since M6-011/M6-012 (Batches 10–11); and `AppHeader`'s
unconstrained-`<select>` fix plus `AppShell`'s `min-w-0` fix (Batch 12,
M5-023) are shared layout components that already apply to every
route, including this one. M6-021's own job — confirmed empirically,
not assumed — was to verify these already compose correctly on this
specific route, not to build a new responsive mechanism.

**One genuine, positive finding, mirroring Batch 12's own "table needs
its own scroll container" result**: with all 3 saved scenarios
selected, the Scenario Comparison table's own container genuinely
requires horizontal scrolling at 375px (`scrollWidth` 365 >
`clientWidth` 309 inside the container), while the _page_ itself
never overflows (confirmed both by direct measurement and by scrolling
the container to its end and confirming the last column, "Scenario
Charlie," becomes visible) — the `overflow-x-auto` wrapper is working
exactly as designed, containing overflow locally rather than widening
the page, the same property Batch 12 verified for the Dashboard's own
Portfolio Composition table.

**No Simulation source file was changed for this batch — the entire
diff is a single test file.** Unlike Batch 12, which needed real CSS
fixes to `AppHeader.tsx`/`AppShell.tsx`, this batch found nothing to
fix. `tests/e2e/responsiveLayout.spec.ts` (the same file Batch 12
created, not a new Simulation-specific file — this is a cross-cutting
"no horizontal overflow" concern, not a milestone-specific one, the
same "one place per concern" discipline this engagement applies
elsewhere) gained 4 new tests: three per-viewport (375/768/1280px) "no
horizontal page scrolling on the Simulation Workspace" tests mirroring
the Dashboard section's own structure exactly, plus one dedicated
"Scenario Comparison table scrolls within its own container" test
mirroring the Dashboard's own "Portfolio Composition table" test. The
file's own header comment was extended (not replaced) to document both
M5-023 and M6-021 side by side, and to state explicitly that the
Simulation section is a documented negative result rather than an
omission.

**Manual verification was the same real-browser-rendering process used
to write the regression tests above** — no separate informal check was
performed and discarded; the Playwright assertions themselves are the
verification, executed against the real dev server with real user
interactions (portfolio creation, scenario saving, checkbox selection),
not against mocked data.

**Scope discipline**: `git diff --stat -- engine/ services/ types/
stores/ app/ features/` is completely empty — no Engine, Service,
Type, Store, App route, or Feature component file was touched at all,
the cleanest architecture diff of any batch in this milestone so far
(cleaner than Batch 13/17's own "zero engine/services/types/stores"
result, since even `app/`/`features/` are untouched here).
...
| `pnpm lint`                  | ✅ Pass                                                                                                           |
| `pnpm format:check`          | ✅ Pass                                                                                                           |
| `pnpm test` (Vitest)         | ✅ Pass, 1287/1287 (unchanged — no unit-tested source file was touched)                                           |
| `pnpm test:coverage`         | ✅ 96.31% statements / 91.16% branches / 100% functions / 99.08% lines (project-wide, unchanged from Batch 19)    |
| `pnpm build`                 | ✅ Pass — `/simulation` unchanged at 109 kB (no source file changed)                                              |
| `pnpm test:e2e` (Playwright) | ✅ Pass, 39/39 (4 net new: 3 per-viewport Simulation overflow checks + 1 Comparison table scroll-container check) |

**Architecture audit**: `git diff --stat -- engine/ services/ types/
stores/ app/ features/` empty. The only file in this batch's entire
diff is `tests/e2e/responsiveLayout.spec.ts`.

**Traceability**: M6-021's Dependencies (M6-020) were satisfied as of
Batch 19; its Description ("Optimize Simulation Workspace for desktop,
tablet and mobile") is satisfied — confirmed, not assumed — by
real-browser verification at all three named screen classes finding no
overflow anywhere; its DoD ("Simulation tools remain usable on
supported screen sizes") is satisfied by that same verification plus 4
new permanent regression tests guarding both properties (no page-level
overflow at any width; the comparison table's own container correctly
scrolls locally when genuinely too wide) against future regression.
...
### Batch 22 — Unit Tests (M6-023)

**Repository re-sync required before this batch began, per
instruction.** `git ls-remote origin main` showed the real tip was
`0d1d828` — "feat(ui): complete simulation accessibility review
(M6-022)." `git checkout -B claude/profitpilot-repo-review-nty3yy
origin/main` reset the local branch directly to that verified tip;
`git status --short` confirmed a clean working tree before any
implementation began. No mid-batch working-tree reset occurred this
batch either — the fourth consecutive batch to complete without one.

**Dependencies satisfied**: M6-022 (Accessibility Review) is
synchronized to GitHub as of Batch 21.

**M6-023's own text names no `Cover`/`Requirements` checklist at
all** — unlike its closest Dashboard counterpart, M5-025 ("Create
Dashboard Component Tests," Batch 15 of Milestone 5), which named 8
explicit `Cover` items. M6-023's Description ("Create component and
store tests") and DoD ("Critical simulation components are covered")
are the sole text. Given this, the same precedent every other
un-mocked/under-specified Milestone 6 task has established governs:
the literal wording is the sole scope, and — following Batch 15's own
already-approved method for the shape-identical Dashboard task — this
batch is an **audit** of the existing suite (built incrementally,
component-by-component and action-by-action, across Batches 1–21),
not a from-scratch build.

**Confirmed 1:1 test coverage for every Simulation component and
utility before looking for gaps**, the same `ls`-comparison method
Batch 15 used for the Dashboard: all 9 `features/simulation/components/`
files have their own dedicated test file; all 5 `features/simulation/utils/`
files do too. `stores/simulationStore.test.ts` has its own dedicated
`describe` block for every one of its 14 real actions (`setCurrentScenario`,
`runSimulation`, `runPortfolioActionSimulation`, `runTimelineProjection`,
`lastMetadata`, `warnings`, `saveCurrentScenario`, `deleteSavedScenario`,
`toggleComparisonSelection`, `setPreviewMode`, `reset`, independence
from the Portfolio Store, `loadSavedScenario`, `duplicateSavedScenario`).

**Coverage-tool numbers alone were not treated as sufficient
evidence — the same discipline that led Batch 15 to find real gaps
despite already-decent Dashboard coverage.** A precise `lcov.info`
branch-level audit (`BRDA` records, not the coarser per-line `DA`
records, which mis-flagged several already-100%-covered lines as
false positives due to source-map duplication) found exactly 5
individually-flagged uncovered branches across the entire Simulation
feature: `ExportSimulation.tsx:44`, `SaveSimulationForm.tsx:109`,
`ScenarioSummary.tsx:123`, `ScenarioSummary.tsx:150`, and
`SimulationWarnings.tsx:47` — `stores/simulationStore.ts` itself has
zero uncovered branches. Each of the 5 was individually read in
context and confirmed **genuinely, structurally unreachable**, not
merely untested: `ScenarioSummary.tsx`'s own parent component already
gates `<PriceOrInterestSummary />`/`<PortfolioActionSummary />` on
`currentResult !== null`/`portfolioActionPreview !== null` respectively
(real since Batch 8's own "both sections render simultaneously"
finding), making each child's own internal `if (X === null) return
null;` redundant by construction; `ExportSimulation.tsx`/
`SaveSimulationForm.tsx`'s own guards are the same already-documented
"defensive-but-unreachable-in-practice" pattern their own header
comments already call out; `SimulationWarnings.tsx:47`'s final `?? null`
fallback in a 3-way nullish chain is unreachable for the same
parent-guard reason (`if (currentResult === null && portfolioActionPreview
=== null) return <placeholder>;` already rules out the only state that
would reach it). None were force-tested merely to move a percentage —
doing so would mean calling non-exported inner functions directly,
bypassing the real component API, the same discipline against
fabricating coverage this project has followed throughout.

**One real, genuine, coverage-tool-invisible gap was found and
closed — the same class of finding Batch 15 made for the Dashboard's
own "Critical Health Factor"/"Zero debt" `Cover` items, found here by
reasoning about domain scenarios rather than reading percentages.**
`buildSimulationWarnings.ts`'s own `Number.isFinite(simulatedHealthFactor)`
guard — which exists specifically to stop a zero-debt portfolio's real,
computed `Infinity` Health Factor (M2-009's own documented design)
from ever satisfying `Infinity < target` and producing a nonsensical
"Unsafe Health Factor" warning for the _safest_ possible position — had
zero test coverage of its own `false` path, invisible to `v8`'s branch
counter because a short-circuited `&&` chain's compound truth value was
already separately exercised by other tests (target-null and
below-target cases), satisfying the tool's own branch-pair count
without ever actually passing a non-finite value through. Closed with
2 new tests: a focused unit test (`buildSimulationWarnings.test.ts`)
passing `Infinity` directly, and a real end-to-end test
(`SimulationWarnings.test.tsx`) driving an actual zero-debt `Portfolio`
through the real Store's `runSimulation`, confirming the rendered
component shows "No warnings for this simulation." rather than a false
alert.

**Zero production code changed** — the cleanest possible diff for any
batch in this milestone: `git diff --stat -- engine/ services/ types/
stores/ app/ features/` is empty. Only 2 test files were touched.
...
| `pnpm lint`                  | ✅ Pass                                                                                                                                                                                                                        |
| `pnpm format:check`          | ✅ Pass                                                                                                                                                                                                                        |
| `pnpm test` (Vitest)         | ✅ Pass, 1292/1292 (2 net new)                                                                                                                                                                                                 |
| `pnpm test:coverage`         | ✅ 96.31% statements / 91.16% branches / 100% functions / 99.08% lines (project-wide, unchanged — both new tests exercise already-technically-covered code paths, a real behavioral gap invisible to the coverage tool itself) |
| `pnpm build`                 | ✅ Pass — unchanged (no production file touched)                                                                                                                                                                               |
| `pnpm test:e2e` (Playwright) | ⚠️ 40/48, unchanged from Batch 21 — the same 8 pre-existing, environment-scoped failures (already confirmed unrelated via a clean-baseline comparison and accepted by explicit instruction), no new failures                   |

**Architecture audit**: `git diff --stat -- engine/ services/ types/
stores/ app/ features/` empty. `git status --porcelain` shows exactly 2
modified test files, no new or deleted files.

**Traceability**: M6-023's Dependencies (M6-022) were satisfied as of
Batch 21; its Description ("Create component and store tests") is
satisfied by the pre-existing, incrementally-built 1:1 component/utility
test coverage plus the Store's own 14-action `describe` coverage,
confirmed via direct audit rather than assumed; its DoD ("Critical
simulation components are covered") is satisfied by that same audit
plus the one real, closed, previously-invisible gap (the zero-debt
Infinity Health Factor guard), verified by both a focused unit test and
a real end-to-end component test.

---

### Batch 23 — Integration Tests (M6-024)

**Repository re-sync required before this batch began, per
instruction.** `git ls-remote origin main` showed the real tip was
`7cba787` — "test(simulation): complete unit test review (M6-023)."
`git checkout -B claude/profitpilot-repo-review-nty3yy origin/main`
reset the local branch directly to that verified tip; `git status
--short` confirmed a clean working tree before any implementation
began. No mid-batch working-tree reset occurred this batch either —
the fifth consecutive batch to complete without one.

**Dependencies satisfied**: M6-023 (Unit Tests) is synchronized to
GitHub as of Batch 22.

**Found and followed the exact, already-established integration-test
precedent rather than inventing a new convention** —
`tests/integration/dashboard/dashboardWorkflows.test.ts` (M5-026,
Batch 16), `tests/integration/portfolio/portfolioWorkflows.test.ts`
(M4-018), and `tests/integration/services/coreWorkflows.test.ts`
(M3-014) all establish the same shape: no React rendering at all, one
`describe` block per item in the task's own text list, chaining real,
non-mocked Store calls across multiple sequential steps in a single
continuous test. `dashboardWorkflows.test.ts`'s own header comment
states the reasoning this batch reuses verbatim: per-action unit tests
(`tests/unit/stores/simulationStore.test.ts`'s own 14-action suite,
confirmed comprehensive as of Batch 22) already exhaustively cover
each Store action in isolation; what an integration test adds is
following one scenario through a realistic multi-step workflow —
create → edit → compare → save → load — in one continuous test, which
no per-action unit test exercises end to end. A new
`tests/integration/simulation/simulationWorkflows.test.ts` follows
that precedent exactly (no such directory existed yet).

**All 5 items in M6-024's own "Test" list, one `describe` block each
(10 tests total):**

- **Scenario creation** — a real price scenario computes a real,
  Service-derived `currentResult` (cross-checked against known
  arithmetic: 2 BTC × $60,000 − $20,000 debt = $100,000); a real
  portfolio-action scenario computes independently, confirming
  `currentScenario`/`currentResult` stay untouched — M6-003's own
  "completely independent" DoD, re-confirmed at the workflow level.
- **Scenario editing** — changing an active price scenario's BTC Price
  re-computes a genuinely different result, not the stale prior one
  (the same live-update principle `ScenarioBuilder.tsx` itself
  follows: "no Calculate button"); changing an active interest
  scenario's Borrow Rate re-triggers the timeline projection with the
  new rate, confirmed by a strictly higher final interest cost, not an
  unchanged one.
- **Scenario comparison** — toggling `comparisonSelection` on/off
  repeatedly never recalculates a saved scenario's own result, verified
  at the strongest possible level: reference identity (`===`), not just
  equality, since `toggleComparisonSelection` never touches
  `savedScenarios` at all (confirmed by reading the Store's own
  source) — M6-010's own DoD ("without recalculation") satisfied
  structurally; a second test confirms the selected subset reflects
  exactly the chosen scenarios regardless of save order.
- **Simulation saving** — a saved record carries the exact
  `scenario`/`result`/`metadata` active at the moment of saving
  (`result`/`metadata` asserted by reference, not just value, since
  `saveCurrentScenario` stores the live object directly); saving is
  confirmed not to clear or mutate the still-active current scenario.
- **Simulation loading** — the core M6-016 round-trip property, chained
  across a realistic multi-step workflow: save a scenario, diverge to a
  different one, then load the original back and confirm the _exact_
  original result and Formula Version/metadata are restored (by
  reference for metadata) — not a value recalculated against the
  portfolio at load time, and not the diverged scenario's own metadata.
  A second test confirms `loadSavedScenario` preserves whatever
  `comparisonSelection` was already set — Load and Compare are
  independent concerns, confirmed by reading `loadSavedScenario`'s own
  source (it never touches `comparisonSelection`).

**Zero production code changed** — `git diff --stat -- engine/
services/ types/ stores/ app/ features/` is empty. Only one new file
was added: `tests/integration/simulation/simulationWorkflows.test.ts`.
...
| `pnpm lint`                  | ✅ Pass                                                                                                                                                                      |
| `pnpm format:check`          | ✅ Pass                                                                                                                                                                      |
| `pnpm test` (Vitest)         | ✅ Pass, 1302/1302 (10 net new)                                                                                                                                              |
| `pnpm test:coverage`         | ✅ 96.31% statements / 91.16% branches / 100% functions / 99.08% lines (project-wide) — unchanged from Batch 22 (no source line changed)                                     |
| `pnpm build`                 | ✅ Pass — unchanged (no production code touched)                                                                                                                             |
| `pnpm test:e2e` (Playwright) | ✅ Pass, 48/48 — the 8 pre-existing failures Batch 21/22 documented did not reproduce this run, consistent with environment-scoped flakiness rather than a deterministic bug |

**Architecture audit**: `git diff --stat -- engine/ services/ types/
stores/ app/ features/` empty; `git status --porcelain` shows exactly
one new, untracked directory (`tests/integration/simulation/`) and no
modified files at all.

**Traceability**: M6-024's Dependencies (M6-023) were satisfied as of
Batch 22; its Description ("Test: Scenario creation, Scenario editing,
Scenario comparison, Simulation saving, Simulation loading") and all 5
named items are each addressed individually above; its DoD
("Simulation workflows operate correctly") is satisfied literally —
every test chains multiple sequential state transitions on the same
scenario/Store and asserts correctness at each step, not just a single
before/after snapshot.

---

### Batch 24 — End-to-End Tests (M6-025)

**Repository re-sync required before this batch began, per
instruction.** `git ls-remote origin main` showed the real tip was
`5f1c4f0` — "test(simulation): implement simulation integration tests
(M6-024)." A stray, stale set of already-upstream duplicate changes
(the Batch 21 color-contrast/`scope="col"` fixes, byte-identical to
what `origin/main` already carries) was found in the local working
tree, confirmed via direct comparison against `origin/main`'s own
content, and discarded; `git checkout -B
claude/profitpilot-repo-review-nty3yy origin/main` then reset the
local branch directly to the verified tip; `git status --short`
confirmed a clean working tree before any implementation began.

**Dependencies satisfied**: M6-024 (Integration Tests) is synchronized
to GitHub as of Batch 23.

**Followed `tests/e2e/dashboardWorkflows.spec.ts`'s (M5-027) own
established convention exactly**: one `test('Cover: <item>', ...)`
block per item in this task's own "Cover" list, real in-app link
navigation between steps (never a mid-flow `page.goto()`, which
reloads the document and wipes the in-memory Zustand store — Conflict
B), the same `fillByLabel`/portfolio-creation helper shape. New
`tests/e2e/simulationWorkflows.spec.ts`.

**All 7 named `Cover` items, one test each:**

- **Open workspace** — creates a portfolio, navigates to Simulation via
  the sidebar, confirms the heading and the Scenario Controls region
  render.
- **Create simulation** — enters a real BTC Price and confirms
  Simulation Results shows the real, correctly-computed equity ($2
  BTC × $65,000 − $20,000 debt = $110,000).
- **Modify assumptions** — confirms Simulation Assumptions' own "Rate
  Assumptions" section is absent for a plain price scenario, then
  appears with the correct new Borrow Rate once entered — the genuine
  behavior change this item names, not merely a static render.
- **Compare scenarios** — saves two scenarios, selects both, confirms
  both column headers and both real computed equity values render in
  the comparison table.
- **Save** — fills and submits the Save Scenario form, confirms the
  "Saved." confirmation and the new entry in the saved-scenario list.
- **Reload** — saves a scenario, diverges to a different one (a real,
  later, unrelated price change), confirms the diverged result
  displays, then clicks Load and confirms the _original_ saved result
  is restored, not the diverged one — the core M6-016 round-trip
  property, driven through the real, compiled browser this time.
  Deliberately does not assert the Scenario Builder's own input field
  repopulates, since `ScenarioComparison.tsx`'s own header comment
  already documents that as a known limitation, not a bug this test
  should invent an expectation against.
- **Export** — clicks "Export JSON" and asserts a real browser download
  fires with the correct filename, using Playwright's own
  `page.waitForEvent('download')` API — the first e2e-level download
  assertion in this codebase (no prior precedent existed to follow, so
  Playwright's own documented, official mechanism was used directly,
  not an invented one).

**Two real, found-not-assumed bugs in the first draft, both fixed** —
the same class of finding `dashboardWorkflows.spec.ts`'s own Batch 17
write-up documented for its own first draft: `getByText('$120,000.00')`
in the Compare scenarios test was ambiguous against the Simulation
Results panel's own "$X → $Y" arrow-separated text (Playwright's text
matcher spans sibling text nodes around the `aria-hidden` arrow span);
fixed by scoping to `getByRole('cell', ...)` in the comparison table
specifically. `getByText('$40,000.00')` in the Reload test was
ambiguous against a diverged-loss scenario's own "-$40,000.00"
Profit/Loss figure (which contains "$40,000.00" as a literal
substring); fixed with `.first()`, since "Portfolio Value" renders
before "Profit/Loss" in `ScenarioSummary.tsx`'s own `METRIC_ORDER`.

**Unlike M5-027, M6-025's own DoD names no "supported viewport sizes"
clause** — its wording is simply "Critical Simulation Workspace
workflows pass successfully," so no dedicated viewport-parametrized
loop was added here; `tests/e2e/responsiveLayout.spec.ts` (Batch 20,
M6-021) already separately proves the Simulation Workspace itself
never overflows at any of the three supported widths, and repeating
all 7 Cover items three times over would triple this file's runtime
for no new signal — the same reasoning `dashboardWorkflows.spec.ts`'s
own header comment already applied to its own (present, for M5-027)
viewport clause.

**A stale `.next` production build cache caused an unrelated
`pnpm test:e2e` startup failure** (`routesManifest.dataRoutes is not
iterable`, then a "no production build found" error once the stale
cache was cleared) — `playwright.config.ts`'s own `webServer` runs
`pnpm start` (a production server), which requires a fresh `pnpm build`
first; not a bug in this batch's own code, resolved by rebuilding
before running the suite.

**Zero production code changed** — `git diff --stat -- engine/
services/ types/ stores/ features/ app/ components/` is empty. Only one
new file was added: `tests/e2e/simulationWorkflows.spec.ts`.
...
| `pnpm lint`                  | ✅ Pass                                                                                                                                  |
| `pnpm format:check`          | ✅ Pass                                                                                                                                  |
| `pnpm test` (Vitest)         | ✅ Pass, 1302/1302 (unchanged — this batch is Playwright-only)                                                                           |
| `pnpm test:coverage`         | ✅ 96.31% statements / 91.16% branches / 100% functions / 99.08% lines (project-wide) — unchanged from Batch 23 (no source line changed) |
| `pnpm build`                 | ✅ Pass — unchanged (no production code touched)                                                                                         |
| `pnpm test:e2e` (Playwright) | ✅ Pass, 55/55 (7 net new) — the previously-documented pre-existing failures did not reproduce this run either                           |

**Architecture audit**: `git diff --stat -- engine/ services/ types/
stores/ features/ app/ components/` empty; `git status --porcelain`
shows exactly one new, untracked file
(`tests/e2e/simulationWorkflows.spec.ts`) and no modified files at all.

**Traceability**: M6-025's Description ("Create Playwright tests") and
all 7 named `Cover` items are each addressed individually above; its
DoD ("Critical Simulation Workspace workflows pass successfully") is
satisfied by 7 passing real-browser tests driving the actual compiled
application, not a simulated or mocked one.

---

### Batch 25 — UI Specification Audit (M6-026) — final Milestone 6 task

**Dependencies satisfied**: M6-025 is synchronized to GitHub as of
Batch 24. This is the last Milestone 6 task per `06_TASKS.md` —
confirmed directly: M6-026 is the final entry before
`06_TASKS.md`'s own "MILESTONE ACCEPTANCE CRITERIA" section closes out
Milestone 6.

**Read `03_UI.md` Page 5 ("Simulation Workspace") in full for the
first time at this granularity** — not just the individual sections
already cited across 24 prior batches (ACCESSIBILITY, EXPORT OPTIONS,
RESPONSIVE), but the entire page, section by section, since this
task's own Description is explicitly a "final implementation audit
against `03_UI.md`," the same scope Batch 18 (M5-028) applied to the
Dashboard. Also re-read `01_PRD.md`'s REQ-004 "Simulation Engine
Specification" chapter in full and re-verified `06_TASKS.md`'s own
Milestone 6 section via a fresh grep for any Page-5 vocabulary that
might have been silently missed.

**Individually verified this task's own 5 named `Review` items:**

- **Layout** — Page 5's own `PAGE LAYOUT` sketch names 5 regions
  (Scenario Controls, Simulation Results, Portfolio Comparison,
  Recommendation, Scenario Summary). The actual build has all of
  these except Recommendation (see the new conflict below), plus 4
  more regions each backed by its own explicit `06_TASKS.md` task not
  named in Page 5's own sketch: Simulation Assumptions (M6-013),
  Simulation Warnings (M6-014), Export (M6-019), and Save/History
  (M6-015/M6-020). Consistent with this engagement's established
  precedent (Conflict #30's own Batch 18 finding) of treating
  `06_TASKS.md` as authoritative over a `03_UI.md` sketch — every
  extra region traces to its own named task, none is invented.
- **States** — the real, in-scope gap this batch found and fixed (see
  below).
- **Terminology** — spot-checked the one place wording could
  plausibly drift: Page 5 calls the interest-rate field "Borrow APR";
  `06_TASKS.md`'s own M6-006 ("Implement Interest Rate Simulation")
  Include list calls it "Borrow Rate," and the actually-built
  `ScenarioBuilder.tsx` label reads "Borrow Rate (0–1)" — confirmed
  matching `06_TASKS.md`'s own wording exactly, the authoritative
  source for this build throughout, not a deviation.
- **Accessibility** — already satisfied by Batch 21 (M6-022)'s own
  real axe-core WCAG AA scans across every Simulation state; no new
  gap found on re-check.
- **Responsiveness** — already satisfied by Batch 20 (M6-021)'s own
  dedicated three-viewport test suite; no new gap found on re-check.

**One real, in-scope, fixable gap found and fixed under "States":**
`stores/simulationStore.ts`'s own `status`/`errors` fields (set
correctly on every `runSimulation`/`runPortfolioActionSimulation`
failure, and already tested at the Store level since Batch 1) were
never read by any component anywhere in the Simulation Workspace —
confirmed by a direct search across every `features/simulation/components/*.tsx`
file. This is genuinely reachable, not defensive-but-unreachable code:
a portfolio with zero collateral and nonzero debt (a valid, creatable
Milestone 4 portfolio state — the same state `DashboardErrorBanner`,
M5-021, already handles for the Dashboard) makes `simulateScenario`'s
own internal baseline re-snapshot fail, and `validateScenarioBuilderInput`
(M6-004) has no rule that could ever catch this, since the problem is
with the _portfolio itself_, not the scenario delta being entered.
Fixed by adding an error-display branch to `ScenarioSummary.tsx`,
reusing `DashboardErrorBanner`'s own `role="alert"`/error-code display
shape, but deliberately omitting its Retry/recovery-copy buttons —
...
DESIGN PHILOSOPHY, already cited throughout this milestone) means the
next input change is the retry; a redundant button would contradict
that principle. Verified with a new unit test using a real zero-
collateral portfolio through `runSimulation` (not a hand-crafted mock
error), and with mandatory manual browser verification: a screenshot
confirms the red-bordered `role="alert"` box renders the real Service
error message and code while every other Simulation section (Simulation
Assumptions, Simulation Warnings, Export, Save, Portfolio Comparison,
Scenario Charts, Scenario Timeline) correctly continues showing its
own unaffected placeholder text, not stale or crashed output.

**One significant new gap found, flagged as a new conflict rather than
built**: Page 5 Section 4 ("RECOMMENDATION") documents a dedicated
region with worked Excellent/Warning/Critical examples and its own
"only one recommendation is displayed" rule. `01_PRD.md`'s REQ-004-A
corroborates this independently in two separate places — its own
"Outputs" list for BTC Price Simulation names "Recommendation" as one
of 8 outputs, and a separate "Simulation Result" fields list also
names "Recommendation" alongside Scenario Name/Simulation ID/Timestamp/
Portfolio Snapshot/Financial Metrics/Risk Metrics/Warnings/Success
Status/Execution Time. A fresh grep of `06_TASKS.md`'s full Milestone
6 section (M6-001 through M6-026, all Descriptions/Includes/DoDs)
confirms zero occurrences of "recommendation" anywhere in it — the one
"Recommendations are transparent" hit in the file belongs to
Milestone 5's own tail-end Acceptance Criteria, not Milestone 6. No
Engine or Service function computes a Simulation-specific recommendation
anywhere in `engine/simulation/` or `services/simulation/scenario.ts`
(Milestone 5's own `generateRecommendationSet`, M5-013/M5-014, is a
Dashboard-scoped Recommendation Engine call with its own separate,
already-known gaps — Conflict #29 — and is not wired to Simulation
results at all). Building this would mean inventing new Engine/Service
logic beyond this M-effort UI-audit task's own scope, and beyond what
any dependency in this task's own chain (M6-025) asked for. Flagged as
Conflict #31 for a product decision (build a real Simulation
Recommendation feature against documented rules, or mark Page 5
Section 4 superseded), not an engineering one — the same handling
Conflict #30 received for the Dashboard's own Page 3 gap.

**Two minor, non-blocking gaps resolved by `06_TASKS.md`'s own
silence, not built**: Page 5's own `INPUT TYPES` note names a Number
Input **and** a Slider for BTC Price; only the Number Input is built,
and M6-004's own literal 6-field Include list names no slider anywhere
— resolved the same way Batch 3's own scoping decision already
resolved every other Scenario Controls field, in favor of
`06_TASKS.md`'s own silence. Page 5 Section 3 ("PORTFOLIO COMPARISON")
also names a Green/Red/Gray difference-highlighting rule for the
comparison table; `06_TASKS.md`'s own M6-010 DoD names no color rule,
so the existing plain-text comparison table (already accessible,
already WCAG-AA-scanned by Batch 21) was left as-is rather than adding
an undocumented color convention.

**Verified the Milestone 6 Acceptance Criteria checklist
(`06_TASKS.md`, end of Milestone 6) directly, all 9 items** —
Simulation Workspace implemented ✓; Scenario Builder completed ✓;
price and interest simulations supported ✓; portfolio action
simulations implemented ✓; multi-scenario comparison completed ✓;
assumptions fully visible ✓; warnings implemented ✓; simulation
persistence completed ✓; export supported ✓. Every item traces to a
specific, already-synchronized batch (M6-001–M6-025), the same
per-item verification discipline Batch 18 applied to Milestone 5's own
12-item checklist.

**Scope discipline**: `git diff --stat -- engine/ services/ types/
stores/` empty. Exactly one component file touched
(`ScenarioSummary.tsx`) plus its one test file — the fix is a
presentational error-display branch reading two already-existing Store
fields, zero new business logic, zero new data flow.
...
| `pnpm lint`                  | ✅ Pass                                                                                                                                         |
| `pnpm typecheck`             | ✅ Pass                                                                                                                                         |
| `pnpm test` (Vitest)         | ✅ Pass, 1303/1303 (1 net new)                                                                                                                  |
| `pnpm test:coverage`         | ✅ 96.32% statements / 91.19% branches / 100% functions / 99.08% lines (project-wide) — slightly improved                                       |
| `pnpm build`                 | ✅ Pass — `/simulation` unchanged at 109 kB                                                                                                     |
| `pnpm test:e2e` (Playwright) | ✅ Pass, 55/55 (unchanged — no e2e test targets the new error branch, consistent with this fix being a Store-state display, not a new workflow) |

**Architecture audit**: `git diff --stat -- engine/ services/ types/
stores/` empty; `git status --porcelain` shows exactly 2 modified
files (`ScenarioSummary.tsx` and its test) and no new or deleted
files.

**Traceability**: M6-026's Description ("Perform a final
implementation audit against `03_UI.md`") and all 5 `Review` items
(Layout, States, Terminology, Accessibility, Responsiveness) are each
addressed individually above; its DoD ("No undocumented UI deviations
remain without explicit approval") is satisfied — every deviation
found is either fixed (the "States" gap) or explicitly documented
(Conflict #31, and the two minor Slider/color-highlighting scope
notes), none silently left unrecorded. **This closes Milestone 6's own
task list**: M6-001 through M6-026 are now all addressed.

---

## Milestone 7 progress

Milestone 6 was confirmed synchronized to GitHub (`origin/main` at
`8c66d2f`) before this milestone began. A full implementation plan was
produced first, per explicit instruction, covering documentation
reading, anticipated conflicts, architectural considerations,
dependencies on prior milestones, a proposed batch grouping, and a
validation strategy — presented for approval before any code was
written. **Approved with one explicit workflow change**: batches for
Milestone 7 group 5–8 related tasks into one "logical feature batch"
rather than continuing Milestone 5/6's one-task-per-batch density —
the plan's own 40-batch, ~1-task-per-batch breakdown was revised down
to 8 coarser batches before Batch 1 began.

**Read ahead of implementation**: `06_TASKS.md`'s own Milestone 7
header (`MILESTONE OBJECTIVE`, `DELIVERABLES`, `IMPLEMENTATION ORDER`)
and all 45 tasks (M7-001–M7-045); `03_UI.md` Page 6 ("Loop Builder")
and Page 7 ("Exit Planner") in full, plus a full 10-page index sweep
(confirming no Recommendation Center page or sidebar entry exists
anywhere); `01_PRD.md` REQ-004-B ("Loop Simulation"), REQ-006
("Recommendation Engine"), REQ-007 ("Exit Strategy Engine," all of
REQ-007-A through REQ-007-E) in full. **Confirmed the Loop/Exit/
Recommendation Engine and Service layers already exist**, built in
Milestones 2-3: `engine/loop/`, `engine/exit/`, `engine/recommendation/`,
`services/loop/strategy.ts` (`planLoopStrategy`, M3-010),
`services/exit/plan.ts` (`planExit`, M3-011),
`services/recommendation/recommendations.ts`
(`generateRecommendationSet`, M3-012) — the exact three Services
M7-001's own Dependencies name. Milestone 7 builds UI/Store on top of
them; it does not build a second calculation path — the same single
most important fact Milestone 6's own opening note established for
Simulation.

**Found several real, plan-relevant gaps before any code was written**
(all documented in the approved implementation plan, repeated here for
the permanent record): `RecommendationRuleConfig`'s conflict #29 gap
becomes load-bearing rather than cosmetic once the Recommendation
Center is reached, since `06_TASKS.md`'s own M7-032 "Filter by" list
names 3 categories (Safety, Interest, Exit readiness) the Engine's own
`UNAVAILABLE_CATEGORIES` list already structurally blocks; `03_UI.md`
Page 6's own "Auto Loop Engine" design (target-Health-Factor/target-
price auto-solving, its own literal "Never ask the user for loop count
by default" DESIGN RULE) directly contradicts `06_TASKS.md`'s M7-008
("Maximum number of loops" as a direct user input) and the already-
built Engine (`LoopStrategyInput.maxLoops` is a mandatory parameter,
not a solved output) — the largest single Page-vs-Tasks gap since
Conflict #30; the Recommendation Center has no page or sidebar entry
anywhere in `03_UI.md`'s 10-page index, the same shape as Conflict #23
(Portfolio List page); Exit Planner's "Target cash proceeds" (M7-021/
M7-022) has no Engine counterpart (`ExitTarget`'s union has exactly 3
variants — `debtBalance`/`healthFactor`/`retainedBtc`), reviving
Conflict #10 as an actual blocking UI gap for the first time. None of
these blocked Batch 1's own Foundation scope; each will be resolved
when the batch that actually reaches it is implemented, per the plan's
own recommendation.

### Batch 1 — Shared Strategy Foundation (M7-001, M7-002, M7-003,

M7-004, M7-005)

**Dependencies satisfied**: M3-010, M3-011, M3-012 (Loop/Exit/
Recommendation Services) are all complete, confirmed by direct
inspection of their own header comments before implementation began.

**M7-001 ("Create Strategy Feature Foundations")**: created
`features/loop-builder/`, `features/exit-planner/`, and
`features/recommendations/`, each with `components/`, `hooks/`,
`services/`, `types/`, `utils/` (all `.gitkeep` placeholders — no task
through this batch needs any of them yet) and a minimal `index.ts`
public entry point. **Deliberately omits `state/` and `tests/`**, even
though M7-001's own literal suggested-structure code block names
both — the identical, already-established deviation
`features/simulation/index.ts`'s own header comment documents for
M6-002: every Store in this codebase lives in the one top-level
`stores/` directory, every test file lives in the one top-level
`tests/` directory (both from M1-003), never nested inside a feature.

**M7-002 ("Create Shared Strategy View Models")**: new
`types/strategy.ts` (added to `types/index.ts`'s barrel). Defines
`StrategyBaseline` (`PortfolioSummary` + `btcExposure` — covers 9 of
M7-003's own metrics, 8 directly from `calculatePortfolioSummary`,
M3-005, the 9th from `calculateExposure`, F-002), `StrategyComparisonResult`
(`before`/`after` `StrategyBaseline`, `after: null` exactly when
infeasible — mirrors `ExitPlanResult.after`'s own convention),
`StrategyCostItem` (`label`/`amount`/`unavailableReason` — the same
itemize-with-reason convention `UnavailableLoopCost`/
`UnavailableExitCost` already established at the Engine layer for
conflict #8), and `StrategyWarning` (for M7-005, below). "Formula
references"/"Engine version" reuse `ServiceMetadata` directly — no new
type invented. **Lives in the top-level `types/` directory, not inside
any one feature** — `04_BUILD_GUIDE.md`'s own "TYPES DIRECTORY" note
already names this directory's purpose exactly, and M7-003's own DoD
requires genuine cross-feature reuse (Loop Builder AND Exit Planner),
the one deliberate exception to this codebase's usual "each feature
owns its own thin utility layer" preference.

**M7-003 ("Create Shared Before-and-After Comparison Component")**:
new `components/strategy/StrategyComparison.tsx`, rendering all 9
named metrics from a `StrategyComparisonResult`, zero recalculation.
**Only 5 of the 9 metrics are classified "Improved"/"Worsened"** —
Health Factor, LTV, Debt, Interest cost, Liquidation price — each
backed by a documented, universal Formula-Engine safety direction
(F-022's own liquidation boundary makes higher Health Factor always
safer, LTV moves inversely by definition, less Debt/Interest cost are
definitionally smaller liabilities, a lower Liquidation price means
more room before liquidation). The remaining 4 (Collateral, Net
equity, Leverage, BTC exposure) are deliberately left unclassified:
Loop Builder's own purpose is to increase them, Exit Planner's own
purpose is to decrease them, so a fixed "up/down is good" label would
be an invented judgment, not a documented one. "Without relying on
color alone" is satisfied by a visible text tag alongside the number,
not a colored number alone.

**M7-004 ("Create Shared Strategy Assumptions Panel")**: new
`components/strategy/StrategyAssumptionsPanel.tsx`, directly
generalizing `SimulationAssumptions.tsx` (M6-013) — same Market
price/Protocol parameters/Formula version content, extended with
"Borrow Rate" as its own line (currently identical to Protocol
Parameters' own value; will diverge once a Loop-specific override
exists, mirroring Simulation's own "Rate Assumptions" vs. "Protocol
Parameters" distinction), "Gas estimate" added to the already-
established Fees/Slippage conflict #8 note, a caller-supplied
`timeHorizonLabel` (no shared time-horizon concept exists across
Loop/Exit, so this component renders whatever label each tool
supplies, or omits the row for `null`), and a new "Manual-Data Status"
line (`Portfolio.marketUpdatedAt` + the same Manual Mode framing
`buildDataFreshnessIndicators.ts`'s own `REFRESH_NOTE` established,
M5-017) — the one Display item `SimulationAssumptions.tsx` never
built.

**M7-005 ("Create Shared Strategy Warning System")**: `StrategyWarning`
type (`types/strategy.ts`) with all 8 named categories and a
`cause`/`severity`/`suggestedResponse` shape — richer than the generic
`ServiceWarning` (`code`/`message`), which was never designed to carry
a severity or suggested response, so extending it would have been
misusing a type built for a different purpose rather than reusing one.
New `components/strategy/StrategyWarnings.tsx` renders any
`StrategyWarning[]` consistently: `'error'` severity gets `role="alert"`
(assertive), `'warning'` gets `role="status"` (polite) — the same
assertive/polite split established since Batch 13 (M5-021), applied
here for the first time to a list where both severities can appear
together. Zero warnings renders positive confirmation text, the same
"always-visible section" convention every prior warnings component in
this codebase follows. **Generates no real warnings this batch** — no
Loop/Exit Store exists yet (M7-007 arrives in Batch 2, M7-020 in Batch
4); M7-013 and M7-027 both declare an explicit M7-005 dependency for
exactly the batch where real Engine findings get mapped into this
shape.
...
**A second real, closeable gap was found by reading the corrected
coverage output, not merely accepting its top-line percentage** — the
same BRDA-level scrutiny Milestone 6 Batch 22 established.
`components/strategy/format.ts`'s `formatLeverage` had an untested
`!Number.isFinite` guard branch. Unlike a purely defensive,
unreachable case, a zero-net-equity portfolio (collateral value equal
to debt value — a real, creatable Milestone 4 state) makes `leverage`
genuinely `Infinity`, so a test was added exercising exactly that
state through `StrategyComparison` rather than left as an accepted
gap. `formatCurrency`'s own equivalent guard was left untested and
documented as genuinely defensive-but-unreachable: no currency field
this component renders (collateral/debt/net equity/interest
cost/liquidation price) can reach a non-finite value through real
Engine arithmetic, unlike leverage.

**Also observed, not a bug**: the coverage text-table reporter omitted
2 of the batch's 4 new component files from its terminal summary
(`StrategyAssumptionsPanel.tsx`, `StrategyWarnings.tsx`) even though
both are fully exercised — confirmed via direct `lcov.info` inspection
(`FNH`/`LH`/`BRH` all at 100% for both). A text-reporter display quirk,
not a real coverage gap — the same "the text table isn't the full
truth, `lcov.info` is" lesson Batch 22 already established, reapplied
here to a different reporter quirk.

**Scope discipline**: `git diff --stat — engine/ services/ stores/`
empty — zero Engine/Service/Store changes. `types/index.ts` gained
exactly one new barrel export line (M7-002's own literal scope).
`vitest.config.ts` gained exactly the 4 coverage-include entries
above. Every other change is a new, untracked file.
...
| `pnpm lint`                  | ✅ Pass                                                                                                                                                                 |
| `pnpm typecheck`             | ✅ Pass                                                                                                                                                                 |
| `pnpm test` (Vitest)         | ✅ Pass, 1320/1320 (17 net new)                                                                                                                                         |
| `pnpm test:coverage`         | ✅ 96.32% statements / 91.35% branches / 100% functions / 99.11% lines (project-wide) — improved from Batch 25, `components/strategy` itself at 96.22%/95.91%/100%/100% |
| `pnpm build`                 | ✅ Pass — `/loop-builder` and `/exit-planner` unchanged at 0 B (nothing imports the new shared code into a route yet)                                                   |
| `pnpm test:e2e` (Playwright) | ✅ Pass, 55/55 (unchanged — no new route-level UI exists yet for this foundation batch to test)                                                                         |

**Architecture audit**: `git diff --stat — engine/ services/ stores/`
empty; `git status --porcelain` shows 2 modified files
(`types/index.ts`, `vitest.config.ts`) and 6 new untracked directories
(`features/loop-builder/`, `features/exit-planner/`,
`features/recommendations/`, `components/strategy/`,
`tests/unit/components/strategy/`, plus `types/strategy.ts`).

**Traceability**: all 5 tasks' own literal DoD text is satisfied
individually above — M7-001 ("clear public entry point... isolated
from unrelated features"), M7-002 ("stable UI-ready models without
recalculating financial values"), M7-003 ("supports Loop Builder and
Exit Planner results" — generically, not yet wired to either, which
Batches 2 and 4 will prove by actually rendering it), M7-004 ("inspect
the assumptions behind every strategy result"), M7-005 ("Loop and exit
workflows present warnings consistently" — the shared shape/renderer
exists now, real generation and cross-tool consistency will be proven
once both tools use it).
...
### 31. `03_UI.md` Page 5 ("Simulation Workspace") documents a "Recommendation" region with no Engine/Service-layer support anywhere — found while performing the final M6-026 audit (Milestone 6 Batch 25)

**`03_UI.md` Page 5's own `PAGE LAYOUT` sketch names 5 regions**:
Scenario Controls, Simulation Results, Portfolio Comparison,
**Recommendation**, and Scenario Summary. Its own "SECTION 4
RECOMMENDATION" gives three fully worked examples (Excellent, Warning,
Critical) and a specific display rule: "Only one recommendation is
displayed" (the highest-priority one, implied by the worked examples'
own severity ordering).

**Corroborated independently in two separate places in `01_PRD.md`'s
own REQ-004 chapter**, not just this one `03_UI.md` page: REQ-004-A's
own "Outputs" list for BTC Price Simulation names "Recommendation" as
one of 8 outputs (alongside Portfolio Value, Net Worth, Debt, Health
Factor, Liquidation Price, Effective Leverage, Interest Cost — all 7 of
which are already built and displayed by `ScenarioSummary.tsx`); and a
separate "Simulation Result" fields list also names "Recommendation"
alongside Scenario Name, Simulation ID, Timestamp, Portfolio Snapshot,
Financial Metrics, Risk Metrics, Warnings, Success Status, and
Execution Time.

**Zero Engine or Service support exists for this anywhere in the
codebase** — confirmed by a fresh grep of `06_TASKS.md`'s full
Milestone 6 section (all of M6-001 through M6-026's own Descriptions,
Includes, and DoDs): zero occurrences of "recommendation" appear
anywhere in it. The one "Recommendations are transparent" hit found in
the file belongs to Milestone 5's own tail-end Acceptance Criteria, not
Milestone 6. `engine/simulation/` has no recommendation-producing
function, and `services/simulation/scenario.ts`'s own `simulateScenario`
returns no such field. Milestone 5's own `generateRecommendationSet`
(M5-013/M5-014, `engine/recommendation/`) is a real, already-built
Recommendation Engine call — but it is Dashboard-scoped (consumes the
_current_ portfolio state only) and is not wired to Simulation results
in any way; it also carries its own separate, already-documented input
gap (Conflict #29 — 5 of `RecommendationRuleConfig`'s 7 fields have no
portfolio-level source).

**Why this was not built**: `06_TASKS.md` — the document this entire
engagement has followed task-by-task, resolving every prior
`03_UI.md`-vs-`06_TASKS.md` mismatch in its favor (Conflict #30's own
Batch 18 precedent) — assigns no task anywhere to a Simulation
Recommendation feature. Building one now would mean inventing new
Engine/Service logic (a scoring or rule-selection function, its inputs,
its severity thresholds) from scratch, with no documented formula or
rule to follow — the same category of problem Conflict #9 already
identifies for the Dashboard's own Recommendation Engine formula chapter
(F-060–F-069, unassigned in `06_TASKS.md`). This is well beyond the
scope of an "M"-effort UI-audit task (M6-026) whose own dependency
chain (M6-025) never asked for one, and would require fabricating
business logic — explicitly forbidden throughout this engagement.

**No code changes were made.** Flagged for a product decision (assign
a real task and formula for a Simulation Recommendation feature in a
future milestone, or mark Page 5 Section 4 superseded, the same two
...
5. **Milestone 6 — Simulation Workspace is complete and synchronized to
   GitHub.** All 26 tasks (M6-001–M6-026) addressed across Batches 1–25
   ...
   Workspace: M6-021; Accessibility Review: M6-022; Unit Tests: M6-023;
   Integration Tests: M6-024; End-to-End Tests: M6-025; UI
   Specification Audit: M6-026).
   The Simulation
   ...
   established for an un-mocked task. Batch 13 (M6-014, "Implement
   Simulation Warnings") built only 2 of the 6 documented warning
   categories — "Unsafe Health Factor" (reusing
   `Portfolio.settings.safetyTargets?.targetHealthFactor`, the same
   "distance from the user's own configured target, not an invented
   risk band" pattern `features/dashboard/types/riskWarnings.ts`
   (M5-010) already established for Conflict #1) and "Stale prices"
   (reusing `normalizeMarketQuote`'s own documented 5-minute threshold,
   M3-007) — and documented the other 4 ("Near liquidation," "Invalid
   assumptions," "High leverage," "High borrowing cost") as blocked or
   structurally unreachable, re-confirmed by fresh grep rather than
   assumed from Dashboard's own prior M5-010 findings. Needed zero new
   `engine/`/`services/`/`types/`/`stores/` changes — the first fully
   clean architecture diff since Batch 5 — since every value it needed
   already existed and was already exported. Batch 14 (M6-015, "Save
   Simulation") closed a gap explicitly flagged as a forward reference
   three separate times earlier in this milestone (`stores/
simulationStore.ts`'s own Batch 2 header comment,
   `ScenarioComparison.tsx`'s own Batch 9 header comment, and this
   file's own Batch 9/10/11 write-ups) — `SavedSimulation` gained
   `name`/`description`/`portfolioId`, and `saveCurrentScenario` now
   takes a real input instead of no arguments, with `portfolioId`
   supplied as a plain caller value (the same pattern `runSimulation`
   already uses for `portfolio`), keeping the Store's own documented
   independence from `usePortfolioStore` fully intact. "Portfolio
   reference" is stored but not yet displayed or cross-checked against
   drift — that is M6-016's own explicit Requirement, not pre-empted
   here. `ScenarioComparison.tsx` was updated to show the real `name`
   instead of a generic type-only label, verified end-to-end in manual
   browser testing by saving two real, differently-named scenarios and
   confirming both the checkbox list and the comparison table's own
   column headers show the real names. This batch also required a
   mid-batch recovery: after implementation and validation were
   complete, the local working tree unexpectedly reverted to an
   earlier commit, deleting the batch's uncommitted work; a fresh
   `git fetch` confirmed `origin/main` itself was unaffected, the local
   branch was re-synced to it, and the implementation was redone from
   scratch and re-validated, reproducing identical results. Batch 15
   (M6-016, "Load Saved Simulation") added a new `loadSavedScenario`
   Store action that restores a saved scenario's own already-computed
   `scenario`/`result` directly, never recalculating against the current
   portfolio, satisfying the task's own "Preserve original assumptions"
   Requirement literally; a new `portfolioUpdatedAt` snapshot field on
   `SavedSimulation` (this batch's own addition) is compared against the
   live `Portfolio.updatedAt` inside `ScenarioComparison.tsx`'s own new
   `driftNotice` helper to satisfy the second Requirement, "Display if
   portfolio has changed since creation" — kept out of the Store itself
   to preserve its documented independence from `usePortfolioStore`.
   Manual browser verification confirmed both the Load-restoration and
   drift-detection flows end-to-end through real UI interaction,
   including a real portfolio mutation via the Portfolio page's own
   Preview/Apply form flow. This batch also required a mid-batch
   recovery of its own: after implementation, validation, and manual
   browser verification were already complete and the commit was
   approved, the local working tree unexpectedly reverted to an earlier
   commit (`M6-007`), deleting the batch's entire uncommitted
   implementation; a fresh `git fetch`/`git ls-remote` confirmed
   `origin/main` itself was unaffected (still carrying the real Batch 14
   tip, `17f7faa`, with Batches 1–14 intact), the local branch was
   re-synced to it, and the implementation was redone from scratch and
   re-validated, reproducing identical results (1250/1250 tests,
   identical coverage percentages, identical bundle size). Batch 16
   (M6-017, "Duplicate Simulation") added a new `duplicateSavedScenario`
   Store action that reuses `stores/portfolioStore.ts`'s own
   already-approved `duplicate` convention (new identity, `" (Copy)"`
   name suffix, fresh timestamp) verbatim, since M6-017 itself names no
   `Requirements` section to define what "duplicate" should mean; "Load"
   and "Duplicate" both live in `ScenarioComparison.tsx`, the only place
   a saved scenario is ever rendered. This batch's own mandatory manual
   browser verification also found and fixed one real, pre-existing,
   unrelated bug: `ScenarioSummary.tsx`'s warnings list keyed each row by
   `warning.code` alone, which produced a React "duplicate key" console
   error whenever a `ServiceWarning[]` genuinely contained two entries
   with the same code — fixed by keying on `` `${warning.code}-${index}` ``
   instead, the same "found and fixed during mandatory verification"
   precedent Batch 8 already established. This batch also required a
   mid-batch recovery of its own: after implementation, validation, and
   manual browser verification were already complete and the commit was
   approved, the local working tree unexpectedly reverted to an earlier
   commit (`M6-007`), deleting the batch's entire uncommitted
   implementation; a fresh `git fetch`/`git ls-remote` confirmed
   `origin/main` itself was unaffected (still carrying the real Batch 15
   tip, `88e02b0`, with Batches 1–15 intact); the local branch was
   re-synced to it and the implementation was redone from scratch and
   re-validated in full, reproducing identical results (1256/1256 tests,
   identical coverage percentages, identical bundle size). Batch 17
   (M6-018, "Delete Simulation") wired the Store's own already-real
   `deleteSavedScenario` (real since Batch 2/M6-003, never called by any
   UI until now) to a new "Delete" button in `ScenarioComparison.tsx`,
   reusing `app/portfolios/page.tsx`'s own already-approved inline,
   per-row confirmation pattern (M4-012) rather than inventing a second
   confirmation UX — satisfying the DoD ("Deletion cannot occur
   accidentally") with a click-to-request/Confirm-or-Cancel flow. This
   was the first batch since Batch 13 to need zero `engine/`/
   `services/`/`types/`/`stores/` changes at all. This batch also
   required a mid-batch recovery of its own: after implementation,
   validation, and manual browser verification were already complete
   and the commit was approved, the local working tree unexpectedly
   reverted to an earlier commit (`M6-007`), deleting the batch's
   entire uncommitted implementation; a fresh `git fetch`/
   `git ls-remote` confirmed `origin/main` itself was unaffected (still
   carrying the real Batch 16 tip, `7d59a8d`, with Batches 1–16 intact);
   the local branch was re-synced to it and the implementation was
   redone from scratch and re-validated in full, reproducing identical
   results (1261/1261 tests, identical coverage percentages, identical
   bundle size). Batch 18 (M6-019, "Export Simulation") disambiguated
   itself from `services/export/`'s own Milestone-8-reserved scaffold
   (M3-001) before writing any code, confirming via `06_TASKS.md` that
   Milestone 8 owns its own separate "Create Export Service"/"Implement
   Single-Record JSON Export"/"Implement CSV Export" tasks — this
   batch's export logic lives entirely inside the Simulation feature
   instead, the same boundary `utils/portfolioRecoveryExport.ts`
   (M4-017) already drew for its own narrower portfolio-recovery
   export. `SavedSimulation` gained a `metadata: ServiceMetadata | null`
   snapshot (captured internally from the Store's own `lastMetadata`,
   not caller-supplied), needed to satisfy "Timestamp"/"Formula
   version" honestly; `loadSavedScenario` was corrected to restore
   `lastMetadata` from that snapshot instead of clearing it to `null`
   (Batch 15's own prior behavior), which also fixed a latent gap in
   `SimulationAssumptions.tsx`'s own "Formula Version" row silently
   disappearing after every Load — found and fixed during this batch's
   own mandatory manual verification, the same
   "found-and-fixed-during-verification" precedent Batch 8/16 already
   established. Export lives next to "Simulation Results" (a new
   `ExportSimulation.tsx`, operating on `currentScenario`/
   `currentResult`), not inside `ScenarioComparison.tsx`'s saved-row
   list — `03_UI.md`'s own "EXPORT" section names "Scenario Summary" as
   what gets exported, the same result `ScenarioSummary.tsx` already
   renders; exporting an old saved simulation means Loading it first
   (M6-016, this task's own Dependency), then using the same controls.
   This batch also required a mid-batch recovery of its own: after
   implementation, validation, and manual browser verification were
   already complete and the commit was approved, the local working
   tree unexpectedly reverted to an earlier commit (`M6-007`), deleting
   the batch's entire uncommitted implementation (including its two
   new, untracked source files); a fresh `git fetch`/`git ls-remote`
   confirmed `origin/main` itself was unaffected (still carrying the
   real Batch 17 tip, `2ed0ab7`, with Batches 1–17 intact); the local
   branch was re-synced to it and the implementation was redone from
...
'portfolio' | 'name'`) state plus a `<select>` control were added; a
   ...
   each stage. Batch 20 (M6-021, "Responsive Workspace") investigated
   the real, compiled Simulation route at 375/768/1280px via
   Playwright, the same method Batch 12 (M5-023) established for the
   Dashboard, and found zero real overflow bugs — a documented negative
   result, since every layout mechanism this route depends on
   (`app/simulation/page.tsx`'s own `flex-col lg:flex-row` split since
   M6-001; `ScenarioComparison.tsx`'s own `overflow-x-auto` table
   container since M6-010; `ScenarioCharts.tsx`/`ScenarioTimeline.tsx`'s
   own recharts `ResponsiveContainer` usage since M6-011/M6-012; and
   Batch 12's own shared `AppHeader`/`AppShell` fixes, which already
   apply to every route) was already built responsively by the batch
   that introduced it. One genuine positive finding mirrored Batch 12's
   own "table needs its own scroll container" result: with 3 scenarios
   selected, the Scenario Comparison table's own container genuinely
   scrolls at 375px while the page itself never overflows, confirmed by
   direct measurement and by scrolling to reveal the last column. No
   Simulation source file was changed — the entire diff is 4 new
   ...
   re-run until green. Batch 22 (M6-023, "Unit Tests") audited the
   already-incrementally-built test suite rather than building from
   scratch, the same shape as Batch 15's (M5-025) own Dashboard
   equivalent — confirmed 1:1 component/utility test-file coverage (9/9
   components, 5/5 utils) and a dedicated `describe` block for every
   one of the Store's 14 real actions; a precise branch-level `lcov`
   audit found only 5 individually-flagged uncovered branches across
   the whole feature, each investigated and confirmed genuinely,
   structurally unreachable (parent-component guards already prevent
   the state their own internal checks guard against — the same
   accepted "defensive-but-unreachable" pattern several prior batches
   already established), none force-tested merely to move a
   percentage. One real, coverage-tool-invisible gap was found by
   reasoning about a domain scenario rather than reading percentages:
   `buildSimulationWarnings.ts`'s own `Number.isFinite` guard, which
   stops a zero-debt portfolio's real `Infinity` Health Factor from
   ever producing a false "Unsafe Health Factor" warning, had zero
   test coverage of its own false path — closed with a focused unit
   test plus a real end-to-end component test through the actual
   Store. Zero production code changed — the cleanest diff of any
   batch in this milestone, only 2 test files touched. Batch 23
   (M6-024, "Integration Tests") followed the exact
   `tests/integration/` precedent Batch 16 (M5-026) established for
   the Dashboard — no React rendering, one `describe` block per named
   item, chaining real Store actions across multi-step workflows — and
   added a new `tests/integration/simulation/simulationWorkflows.test.ts`
   covering all 5 of M6-024's own named items (Scenario creation,
   editing, comparison, Simulation saving, loading) in 10 tests. The
   Scenario comparison test verified M6-010's own "without
   recalculation" DoD at the strongest possible level — reference
   identity, not just equality — since `toggleComparisonSelection`
   never touches `savedScenarios` at all; the Simulation loading test
   chained a full save → diverge → load workflow confirming the exact
   original result and metadata are restored by reference, not
   recalculated. Zero production code changed — only one new file
   added. Batch 24 (M6-025, "End-to-End Tests") followed
   `dashboardWorkflows.spec.ts`'s (M5-027) own established convention
   exactly — one `test('Cover: <item>', ...)` per named item, real
   in-app link navigation, no mid-flow `page.goto()` — and added a new
   `tests/e2e/simulationWorkflows.spec.ts` covering all 7 of M6-025's
   own named Cover items (Open workspace, Create simulation, Modify
   assumptions, Compare scenarios, Save, Reload, Export) in 7 tests
   against the real, compiled application. Two real ambiguous-text-match
   bugs were found and fixed in the first draft, the same class of
   finding Batch 17 documented for its own first draft. Unlike M5-027,
   M6-025's own DoD names no viewport-size clause, so no dedicated
   parametrized viewport loop was added — `responsiveLayout.spec.ts`
   (Batch 20) already covers that separately. Zero production code
   changed — only one new file added. Batch 25 (M6-026, "UI
   Specification Audit," the final Milestone 6 task) read `03_UI.md`
   Page 5 in full for the first time at this granularity, found and
   fixed one real, in-scope gap (the Simulation Workspace never
   displayed the Store's own already-tested calculation-failure
   `status`/`errors` state — fixed in `ScenarioSummary.tsx`), found one
   significant new cross-document-corroborated gap requiring a new
   numbered conflict rather than an in-scope fix (a missing
   "Recommendation" feature, named in `03_UI.md` Page 5 Section 4 and
   corroborated twice in `01_PRD.md` REQ-004-A, with zero Engine/
   Service-layer support — Conflict #31), and resolved two minor,
   non-blocking gaps (the BTC Price Slider input type, and
   Green/Red/Gray comparison-table difference highlighting) in favor
   of `06_TASKS.md`'s own silence on both, the same precedent this
   audit methodology has applied since Batch 18. This closes Milestone
   6's own task list (M6-001 through M6-026).
6. **Milestone 7 Batch 1 (M7-001–M7-005, Shared Strategy Foundation)
   raised no new numbered conflict, but found and fixed two real,
   closeable gaps during its own validation rather than deferring
   either.** First, a coverage-configuration gap matching Milestone 6
   Batch 10's own precedent exactly: `vitest.config.ts`'s coverage
   `include` list had no entry for `components/strategy/**` or any of
   the three new `features/*` directories, meaning every new file in
   this batch would have been silently unmeasured — fixed by adding
   all four entries before treating the batch's own coverage numbers
   as meaningful. Second, reading the corrected coverage output at
   Batch 22's own BRDA-level rigor surfaced an untested
   `!Number.isFinite` guard in `formatLeverage`
   (`components/strategy/format.ts`) that is genuinely reachable (a
   zero-net-equity portfolio makes leverage a real `Infinity`, not a
   contrived one) — closed with a new test rather than left as an
   accepted gap; `formatCurrency`'s own equivalent guard was left
   untested and documented as honestly unreachable, since no currency
   field this batch's components render can ever be non-finite through
   real Engine arithmetic. Also recorded three deliberate scoping
   decisions: `types/strategy.ts` lives in the top-level `types/`
   directory rather than inside any one feature, the one explicit
   exception to this codebase's "each feature owns its own thin
   utility layer" preference, since M7-003's own DoD requires genuine
   Loop-Builder-and-Exit-Planner reuse; `StrategyComparison.tsx`
   classifies only 5 of its 9 metrics as "Improved"/"Worsened" (Health
   Factor, LTV, Debt, Interest cost, Liquidation price), leaving
   Collateral/Net equity/Leverage/BTC exposure unclassified since Loop
   Builder and Exit Planner deliberately move them in opposite
   directions on purpose; and `StrategyWarnings.tsx` generates no real
   warnings yet, since no Loop/Exit Store exists until Batches 2 and 4
   — M7-013 and M7-027 both declare an explicit M7-005 dependency for
   exactly the batch where real mapping happens. `git diff --stat --
engine/ services/ stores/` empty.
7. **Batch 2 raised no new numbered conflict, but recorded one deliberate
...
8. **Batch 3 raised no new numbered conflict, but recorded two deliberate
...
9. **Batch 4 raised no new numbered conflict, but recorded two
...
10. **Batch 5 (M6-008) raised no new numbered conflict, but found and
...
11. **Batch 6 (M6-006) raised two documented conflicts, neither
    ...
    Engine change.
12. **Batch 7 (M6-007) raised no new numbered conflict.** Its own DoD
...
13. **Batch 8 (M6-009) found and fixed one real display bug during its
...
14. **Batch 9 (M6-010) raised no new numbered conflict, but recorded a
...
15. **Batch 10 (M6-011) found and fixed a significant, milestone-wide
...
16. **Batch 11 (M6-012) raised no new numbered conflict, but recorded
...
17. **Batch 12 (M6-013) raised no new numbered conflict, but recorded
...
calculateExitPosition.ts`) — confirmed by a fresh grep of
    ...
    (Batch 11).
18. **Batch 13 (M6-014) raised no new numbered conflict, but re-derived
    conflict #1's own adjacency to "Near liquidation" and conflict #29's
    own adjacency to "High borrowing cost" fresh for Simulation, rather
    than assuming Dashboard's M5-010 findings transfer unchanged.** Both
    were re-confirmed by grep across `01_PRD.md`/`02_Formulas.md`/
    `04_BUILD_GUIDE.md` before this batch began. "Invalid assumptions"
    was found structurally unreachable (the same class of finding as
    `riskWarnings.ts`'s own "Invalid protocol parameters," but for
    `validateScenarioBuilderInput`'s own validation gate instead of the
    Portfolio schema); "High leverage" was newly confirmed to have no
    documented numeric threshold anywhere (not previously checked by
    any prior Dashboard or Simulation batch). The 2 built cases —
    "Unsafe Health Factor" and "Stale prices" — reused only already-real
    fields (`Portfolio.settings.safetyTargets?.targetHealthFactor`,
    `Portfolio.marketUpdatedAt`, `normalizeMarketQuote`'s own documented
    5-minute threshold), needing zero new Store state and zero
    `engine/`/`services/`/`types/` changes — the first fully clean
    architecture diff since Batch 5.
19. **Batch 14 (M6-015) raised no new numbered conflict, but closed a
    gap explicitly documented as a forward reference three separate
    times earlier in this milestone** (`stores/simulationStore.ts`'s
    own Batch 2 header comment, `ScenarioComparison.tsx`'s own Batch 9
    header comment, and this file's own Batch 9/10/11 write-ups).
    `SavedSimulation` gained `name`/`description`/`portfolioId`;
    `saveCurrentScenario` now takes a `SaveSimulationInput` instead of
    no arguments, with `portfolioId` supplied as a plain caller value —
    the same pattern `runSimulation` already uses for `portfolio`
    itself — keeping the Store's own documented independence from
    `usePortfolioStore` fully intact. "Portfolio reference" is stored
    but not yet displayed or cross-checked against portfolio drift —
    that is M6-016's own explicit Requirement, a separate, later task,
    not pre-empted here. `ScenarioComparison.tsx` was updated to show
    the real `name` instead of a generic type-only label, a minimal,
    directly-enabled consequence of this batch's own new data, not
    scope creep. A mid-batch working-tree reset also required redoing
    this batch's implementation from scratch on a re-synced base — see
    the Batch 14 write-up above for the recovery details.
20. **Batch 15 (M6-016) raised no new numbered conflict, but is the
    first Milestone 6 task since M6-011 to carry a formal `Requirements`
    section, addressed literally rather than folded into a generic
    Description reading.** "Preserve original assumptions" is satisfied
    by `loadSavedScenario` restoring the saved `scenario`/`result`
    directly from `SavedSimulation`, never recalculating against the
    current portfolio — the deliberate alternative (calling
    `runSimulation` again) was rejected because it would silently break
    reproducibility the moment the portfolio drifts. "Display if
    portfolio has changed since creation" is satisfied by a new
    `portfolioUpdatedAt` snapshot field (`SavedSimulation`/
    `SaveSimulationInput`, this batch's own addition, not retroactively
    added to Batch 14) compared against the live `Portfolio.updatedAt`
    inside `ScenarioComparison.tsx`'s own `driftNotice` helper — kept
    out of the Store to preserve its documented independence from
    `usePortfolioStore` (M6-003's own DoD). A saved scenario belonging to
    a different portfolio entirely is flagged with a distinct message,
    since `savedScenarios` is never cleared on portfolio switch
    (M6-003's own independence design). Loading does not resync
    `ScenarioBuilder.tsx`'s own local form fields — a documented, known
    limitation, not an oversight, since that component's `values` state
    has only ever written to the Store, never read `currentScenario`
    back out (true since Batch 3), and resyncing it would be a broader
    architecture change this task's own narrow DoD does not ask for.
    This batch also required a mid-batch recovery: after implementation,
    validation, and manual browser verification were already complete
    and the commit was approved, the local working tree unexpectedly
    reverted to an earlier commit (`M6-007`), deleting the batch's
    uncommitted work; a fresh `git fetch` confirmed `origin/main` itself
    was unaffected, the local branch was re-synced to it, and the
    implementation was redone from scratch and re-validated, reproducing
    identical results.
21. **Batch 16 (M6-017) raised no new numbered conflict — it names no
    `Requirements` section, only a Description and a DoD.** "Duplicate a
    saved scenario for experimentation" and "Copies are fully
    independent" were resolved by reusing
    `stores/portfolioStore.ts`'s own already-approved `duplicate`
    convention (M4-011) verbatim — new identity, `" (Copy)"` name
    suffix, fresh timestamp — rather than inventing a new shape for
    "duplicate" from nothing. "Load" and "Duplicate" both live in
    `ScenarioComparison.tsx`, the only place a saved scenario is ever
    rendered, the same placement reasoning Batch 15 already established
    for "Load." This batch's own mandatory manual browser verification
    also found and fixed one real, pre-existing, unrelated bug:
    `ScenarioSummary.tsx`'s warnings list keyed each row by
    `warning.code` alone, which produced a React "duplicate key" console
    error whenever a `ServiceWarning[]` genuinely contained two entries
    with the same code (observed while typing through intermediate BTC
    Price keystroke values during verification) — fixed by keying on
    `` `${warning.code}-${index}` `` instead, the same
    "found-and-fixed-during-verification" precedent Batch 8 already
    established, and covered by a new regression test. A mid-batch
    working-tree reset also required redoing this batch's
    implementation from scratch on a re-synced base — see the Batch 16
    write-up above for the recovery details.
22. **Batch 17 (M6-018) raised no new numbered conflict — it names no
    `Requirements` section, only a Description ("Delete saved
    simulations with confirmation") and a DoD ("Deletion cannot occur
    accidentally").** `deleteSavedScenario` (`stores/simulationStore.ts`)
    has been real since Batch 2/M6-003, but no UI ever called it until
    this batch — the same "action exists in the Store, waiting for its
    own dedicated UI task" pattern `loadSavedScenario`/
    `duplicateSavedScenario` already followed. "With confirmation"/
    "cannot occur accidentally" was satisfied by reusing
    `app/portfolios/page.tsx`'s own already-approved inline, per-row
    delete-confirmation pattern (M4-012, "Implement Portfolio Archive
    and Delete") rather than inventing a second confirmation UX — only
    the confirm/cancel shape was reused, not the Portfolio page's own
    "replacement portfolio" branch, which has no equivalent concept for
    saved scenarios (no "active saved simulation" a deletion could leave
    dangling). This was the first batch since Batch 13 to need zero
    `engine/`/`services/`/`types/`/`stores/` changes at all. A mid-batch
    working-tree reset also required redoing this batch's
    implementation from scratch on a re-synced base — see the Batch 17
    write-up above for the recovery details.
23. **Batch 18 (M6-019) raised no new numbered conflict, but is the
    first Milestone 6 task confirmed to be a genuinely different concept
    from an existing, similarly-named scaffold rather than a duplicate
    of it.** `services/export/` (M3-001) already exists but is
    intentionally empty, reserved for Milestone 8's own later
    "Create Export Service"/"Implement Single-Record JSON Export"/
    "Implement CSV Export" tasks (confirmed by grep against
    `06_TASKS.md` before writing any code) — a full-application
    backup/restore system, not this task's own narrow "export one
    Simulation's own already-computed result." Building inside
    `services/export/` now would have been inventing Milestone 8's own
    scope ahead of time; this batch's export logic lives entirely in
    the Simulation feature instead (`features/simulation/utils/
exportSimulation.ts` + a new `ExportSimulation.tsx`), the same
    boundary `utils/portfolioRecoveryExport.ts` (M4-017) already drew
    for its own narrower export. `SavedSimulation` gained a
    `metadata: ServiceMetadata | null` snapshot, needed to satisfy
    "Timestamp"/"Formula version" honestly for a _saved_ record — the
    same "each task adds exactly what its own scope requires"
    discipline `portfolioUpdatedAt` (Batch 15) already established.
    `loadSavedScenario` was corrected to restore `lastMetadata` from
    that snapshot instead of clearing it to `null` (Batch 15's own
    prior behavior) — a deliberate, documented correction, not a silent
    regression — which also fixed a latent gap in
    `SimulationAssumptions.tsx`'s own "Formula Version" row (M6-013)
    silently disappearing after every Load, found during this batch's
    own mandatory manual browser verification, the same
    "found-and-fixed-during-verification" precedent Batch 8/16 already
    established. A mid-batch working-tree reset also required redoing
    this batch's implementation from scratch on a re-synced base — see
    the Batch 18 write-up above for the recovery details.
24. **Batch 19 (M6-020) raised no new numbered conflict, but is the
...
25. **Batch 20 (M6-021) raised no new numbered conflict, and found zero
    real bugs — a documented negative result, the same class of finding
    as "no fix needed" rather than "not investigated."** Real-browser
    testing at 375/768/1280px (Batch 12's own method, reused verbatim)
    confirmed the Simulation Workspace was already responsive by
    construction: every layout mechanism it depends on — the sidebar
    split (M6-001), the comparison table's `overflow-x-auto` (M6-010),
    both charts' recharts `ResponsiveContainer` (M6-011/M6-012), and
    Batch 12's own shared `AppHeader`/`AppShell` fixes — was already
    built responsively by the batch that introduced it, not retrofitted
    now. `06_TASKS.md`'s own M6-021 text is much shorter than its
    Dashboard counterpart M5-023 (no `Requirements` section, no
    Desktop/Tablet/Mobile column breakdown), and no `03_UI.md` mockup
    exists for it either — resolved the same way every other un-mocked
    Milestone 6 task has been, treating the literal Description/DoD as
    the sole scope rather than inventing a Requirements list to fill
    the gap. The entire diff is 4 new permanent regression tests added
    to the same `tests/e2e/responsiveLayout.spec.ts` Batch 12 created
...
26. **Batch 21 (M6-022) raised no new numbered conflict, but is the
    ...
    until green.
27. **Batch 22 (M6-023) raised no new numbered conflict, and is the
    cleanest possible diff for its own task — zero production code
    changed, only 2 test files.** `06_TASKS.md`'s own M6-023 text names
    no `Cover`/`Requirements` checklist (unlike its Dashboard
    counterpart M5-025's 8-item list), so — following Batch 15's own
    already-approved precedent for the identically-shaped task — this
    was an audit of the existing, incrementally-built suite (1:1
    component/utility test-file coverage; a dedicated Store `describe`
    block per real action), not a build-from-scratch. A precise
    `lcov` `BRDA` (branch-level, not the coarser line-level `DA`) audit
    found only 5 individually-flagged uncovered branches across the
    entire Simulation feature, each confirmed genuinely, structurally
    unreachable by a parent-component guard already in place (the same
    "defensive-but-unreachable" pattern prior batches already
    established) — none force-tested merely to move a percentage. One
    real, coverage-tool-invisible gap was found by reasoning about a
    domain scenario, the same method Batch 15 used to find the
    Dashboard's own "Critical Health Factor"/"Zero debt" gaps:
    `buildSimulationWarnings.ts`'s own `Number.isFinite` guard (stops a
    zero-debt portfolio's real `Infinity` Health Factor from producing
    a false "Unsafe Health Factor" warning) had zero test coverage of
    its own false path, invisible to the coverage tool because a
    short-circuited `&&` chain's compound truth value was already
    separately satisfied by other tests. Closed with a focused unit
    test plus a real end-to-end component test through the actual
    Store.
28. **Batch 23 (M6-024) raised no new numbered conflict.** Followed
    Batch 16's (M5-026) own `tests/integration/` precedent exactly — no
    React rendering, one `describe` block per named "Test" item,
    chaining real Store actions across multi-step workflows in a single
    continuous test — rather than inventing a new integration-testing
    convention. All 5 of M6-024's own named items (Scenario creation,
    editing, comparison, Simulation saving, loading) are covered by a
    new `tests/integration/simulation/simulationWorkflows.test.ts` (10
    tests). The Scenario comparison "without recalculation" property
    (M6-010's own DoD) was verified at the strongest possible level —
    reference identity (`===`) on the saved result object, confirmed by
    reading `toggleComparisonSelection`'s own source to establish it
    never touches `savedScenarios` — not merely asserted by value
    equality. Zero production code changed; only one new file added.
29. **Batch 24 (M6-025) raised no new numbered conflict.** Followed
    `dashboardWorkflows.spec.ts`'s (M5-027) own established convention
    exactly — one `test('Cover: <item>', ...)` per named "Cover" item,
    real in-app link navigation, never a mid-flow `page.goto()` (which
    reloads the document and wipes the in-memory Zustand store —
    Conflict B). All 7 of M6-025's own named Cover items (Open
    workspace, Create simulation, Modify assumptions, Compare
    scenarios, Save, Reload, Export) are covered by a new
    `tests/e2e/simulationWorkflows.spec.ts` (7 tests) against the real,
    compiled application. Two ambiguous-text-match bugs were found and
    fixed in the first draft (an "$X → $Y" arrow-split match, and a
    "-$40,000.00" substring collision with "$40,000.00") — the same
    class of finding Batch 17 documented for its own first draft, not a
    new pattern. "Export" used Playwright's own official
    `page.waitForEvent('download')` API directly, since no prior e2e
    download-testing precedent existed in this codebase to follow.
    Unlike M5-027, M6-025's own DoD names no viewport-size clause, so
    no dedicated parametrized viewport loop was added —
    `responsiveLayout.spec.ts` (Batch 20, M6-021) already covers the
    Simulation Workspace's own responsive behavior separately, and
    repeating all 7 Cover items three times over would triple this
    file's runtime for no new signal. Zero production code changed;
    only one new file added.
30. **Batch 25 (M6-026, the final Milestone 6 task) raised one new
    numbered conflict (Conflict #31) and recorded two minor,
    non-blocking scope notes.** Read `03_UI.md` Page 5 ("Simulation
    Workspace") in full for the first time at this granularity, the
    same methodology Batch 18 (M5-028) established for the Dashboard.
    Found and fixed one real, in-scope gap under the "States" Review
    item: `stores/simulationStore.ts`'s own `status`/`errors` fields
    (set correctly on every calculation failure since Batch 1, already
    tested at the Store level) were never read by any component in the
    Simulation Workspace — confirmed reachable, not defensive-but-
    unreachable, via a zero-collateral/nonzero-debt portfolio (a valid
    Milestone 4 state) that makes `simulateScenario`'s own internal
    baseline re-snapshot fail in a way `validateScenarioBuilderInput`
    cannot catch. Fixed in `ScenarioSummary.tsx`, reusing
    `DashboardErrorBanner`'s (M5-021) own `role="alert"`/error-code
    display shape, deliberately omitting its Retry button since
    Simulation's own "every input updates live, no Calculate/Retry
    button" design philosophy makes the next input change itself the
    retry. Found one significant new gap requiring a numbered conflict
    rather than an in-scope fix: `03_UI.md` Page 5 Section 4 documents
    a "Recommendation" region with worked Excellent/Warning/Critical
    examples, corroborated twice in `01_PRD.md` REQ-004-A (once in its
    own Outputs list, once in a separate Simulation Result fields
    list) — but zero Engine/Service-layer support exists anywhere for
    it, confirmed by a fresh `06_TASKS.md` grep showing zero
    occurrences of "recommendation" anywhere in the actual 26-task
    Milestone 6 list. Building it would mean inventing Engine/Service
    work beyond this M-effort UI-audit task's own scope — flagged as
...
32. **Batch 2 raised no new numbered conflict.** M5-004's "Portfolio
    ...
    pass unchanged.
33. **Batch 3 raised no new numbered conflict.** M5-005's "Status" Support
...
34. **Batch 4 raised one new conflict (#29)**: `generateRecommendationSet`
    ...
    silently dropped.
35. **Milestone 5 Batch 5 raised no new conflict — it independently
...
36. **Batch 6 raised no new conflict — both of its two deliberately
...
37. **Batch 7 raised no new numbered conflict — it resolved the M5-015
...
38. **Milestone 5 Batch 8 raised no new numbered conflict.** M5-017
...
39. **Milestone 5 Batch 9 raised no new numbered conflict.** M5-019
    ...
    per-item reasoning.
40. **Milestone 5 Batch 10 raised no new numbered conflict.** M5-021's
...
41. **Milestone 5 Batch 11 raised no new numbered conflict.** M5-016's
...
42. **Milestone 5 Batch 12 raised no new numbered conflict, but found
    ...
    write-up above for the full reasoning.
43. **Milestone 5 Batch 13 raised no new numbered conflict, but found
    ...
    per-item reasoning.
44. **Milestone 5 Batch 14 raised no new numbered conflict, but found
    ...
    reasoning.
45. **Milestone 5 Batch 15 raised no new numbered conflict.** M5-025
...
46. **Milestone 5 Batch 16 raised no new numbered conflict.** M5-026
...
47. **Milestone 5 Batch 17 raised no new numbered conflict.** M5-027
    ...
    write-up above for the full reasoning.
48. **Milestone 5 Batch 18 raised one new, significant conflict
...
49. **From Milestone 4 Batch 8, still open**: conflict #28 — M4-013's Dependencies
    ...
    architecture.
50. **Batch 9 raised no new conflict.** Every ambiguity in M4-017's short
    ...
    conflict.
51. **From Batch 6, still open**: conflict #27 — M4-012 never says
...
52. **From Batch 5, still open**: conflict #26 — M4-009's DoD requires
...
53. **From Batch 4, still open**: conflict #25 — M4-008 names "Price"
...
54. **From Batch 3, still open — recurred in Batch 7 with the same
    ...
    manual entry only.
55. **From Batch 2, still open**: conflict #23 — 03_UI.md's own "six
...
56. **From Batch 1, still open**: "Settings" (conflict #22) — M4-001
    ...
    decision.
57. **Conflict #20 remains resolved** (Batch 0) — no longer an open
    item.
58. **From Milestone 3 Batch 9 (Formula Engine numbering — not this
...
59. **Outstanding blockers/conflicts carried forward from Milestone 2**:
...
60. **Revisited in Milestone 2 Batch 7, confirmed still open at the
    ...
    approximation).
61. **From Milestone 3/4, still open — final tally at Milestone 4's
