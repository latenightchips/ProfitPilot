...
| REQ-011 Testing Strategy & Quality Assurance | N/A | `tests/**`, `.github/workflows/ci.yml` | This entire Milestone 9 effort | Implemented | **Batch 11 (M9-057–064) fresh re-check**: 228 unit test files / 2123 tests passing, 151/151 Playwright (incl. 43/43 accessibility) run against the real production RC build — see `docs/DOD_COMPLIANCE_AUDIT.md` §7. Coverage-target and financial-tolerance figures conflict across documents — see `docs/QUALITY_PLAN.md` §7 (a documentation-precision conflict, not resolved by this batch, non-blocking). **CI still does not run `pnpm test:e2e`** — a real, still-open gap, unchanged since M1-008/the Batch 1 audit; classified P2 (non-blocking, documented workaround: the full Playwright suite is run manually as part of every release/batch's own validation pipeline, as this batch's own record shows) — see `docs/DEFECT_CLASSIFICATION.md`'s Release Candidate review. |
...
| REQ-013 Deployment, DevOps & Release Architecture | N/A | `.github/workflows/ci.yml`, `next.config.ts`, `package.json` scripts | CI pipeline itself | Implemented | "Cloud synchronization functions correctly" (Version 1 DoD, `04_BUILD_GUIDE.md`) and "Row Level Security enabled" (Security Checklist) are **Rejected — documented approval**. Security headers (CSP, HSTS, X-Frame-Options, X-Content-Type-Options, Referrer-Policy) were added in Batch 6 (M9-035) and freshly re-verified against a real production server response in Batch 11 (M9-057) — the "real gap" this row previously recorded is closed. No live deployment has occurred (M1-009 explicitly deferred, unchanged) — a self-hostable product with no single owned production domain by design, not an open defect; the release candidate build itself was verified in production mode (`rm -rf .next && pnpm build && next start`, Batch 11). |
...
| REQ-017 Project Conclusion, Definition of Done & Definition of Excellence | N/A | N/A (this is the release gate itself) | `docs/DOD_COMPLIANCE_AUDIT.md`, `docs/DEFECT_CLASSIFICATION.md`, M9-057–064 (Batch 11) | Implemented | Milestone 9's own final gate. `01_PRD.md`'s own REQ-017 Final Acceptance Criteria defines "Version 1.0" as this exact feature set (Portfolio Management, Mathematical Engine, Risk Engine, Simulation Engine, Recommendation Engine, Exit Planner, Dashboard, Scenario Comparison, State Engine, Testing Framework, Security Framework, Documentation, CI/CD, AI Development Workspace, Release Documentation) — not a future live-price-feed version (that is Version 2 scope, named separately throughout `01_PRD.md`). Version 1 Quality Sign-Off completed in Batch 11 — see `PROJECT_STATUS.md`'s Batch 11 write-up and `docs/CHANGELOG.md`. |
...
| Registry self-check | Passing | `tests/unit/engine/formulaCoverage.test.ts` — 228/228 unit test files (2123/2123 tests) pass overall as of Batch 11's fresh re-run (see `docs/DOD_COMPLIANCE_AUDIT.md` §7) |
...
| 10 | Responsive Design, User Flows & Final UI Validation | Cross-cutting — no single module | `tests/e2e/responsiveLayout.spec.ts`, `tests/e2e/accessibility.spec.ts` | Implemented — the 6-route accessibility-coverage gap this row previously recorded was closed in Batch 5 (M9-022–028); `/settings`, `/sign-in`, `/sign-up`, `/reset-password`, `/portfolios`, `/portfolio` are all covered by `tests/e2e/accessibility.spec.ts` (43 tests total), re-confirmed passing in Batch 11 |
