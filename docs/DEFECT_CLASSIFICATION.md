...
|---|---|---|
| CI (`.github/workflows/ci.yml`) does not run the full `pnpm test:e2e` suite | P2 → resolved (R1-3/R2-4) | R1-3 ("Runtime Pinning + Production CI Smoke Gate") added a targeted spec (`tests/e2e/productionSmoke.spec.ts`) to every `ci.yml` run, deliberately not the full suite. R2-4 ("Dependency Security Follow-up + Release E2E Policy") adds the full 150-test suite (all 43 accessibility tests included; excludes only `productionSmoke.spec.ts` itself, already covered separately — see that file's own header comment for why) as a manual, on-demand `workflow_dispatch` job (`.github/workflows/e2e-full.yml`), confirmed by two full local runs to pass cleanly and deterministically with no external-service dependency. Release policy documented in `docs/OPERATIONAL_RUNBOOK.md`'s "Full E2E suite (manual release gate)" section. Still not run on every PR — deliberately: full-suite runtime (~2.5 minutes) and the value of a single, fast, always-on smoke gate over a slower full run on every push don't justify making it blocking. |
| 18 `pnpm audit` advisories (11 high, 7 moderate, 0 critical) | P2 → substantially resolved (R2-4) | The original 18-instance count covered the full dependency tree (including dev/lint/test-only tooling). R2-4 established `pnpm audit --prod` (production tree only) as the ongoing release-gate command — see `docs/SECURITY_REVIEW.md`'s M9-029 section and its own "R2-4 update" for the full policy. That command found 9 production findings, of which R2-4's `pnpm.overrides` (`postcss`/`nanoid`/`brace-expansion`/`fast-uri`) closed 8; the 1 remaining (`sharp`) is confirmed unused (no `next/image` anywhere in this application) and deliberately not overridden (native-binary regression risk for zero benefit) — tracked as TRACK / WAIT FOR UPSTREAM. |
...
| Health Factor risk-band classification (F-026/F-060) not implemented | P3 → resolved (owner decision) | Conflict #1 (`PROJECT_STATUS.md`) is now closed: the owner selected `01_PRD.md` REQ-005-A as the canonical basis, with its overlapping boundaries made explicitly mutually exclusive. `calculateRiskCategory` (F-026) is the one and only Health Factor classification in this codebase; F-060 (`calculateHealthFactorRecommendation`) consumes it directly rather than maintaining a second threshold table, and the Dashboard's Health Factor Status section does the same — see `PROJECT_STATUS.md` Conflict #1's own "Update" for the full mapping. |
...
| Health Factor risk-band classification not implemented | P3 → resolved (owner decision) | Resolved — Conflict #1 closed, per §6 |
