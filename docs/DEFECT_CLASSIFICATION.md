...
|---|---|---|
| CI (`.github/workflows/ci.yml`) does not run `pnpm test:e2e` | P2 → partially resolved (R1-3) | Documented, unchanged from the original M1-008 build through every Milestone 9 audit — until R1-3 ("Runtime Pinning + Production CI Smoke Gate"), which added exactly the recommended `pnpm exec playwright install --with-deps chromium` step plus one targeted spec (`tests/e2e/productionSmoke.spec.ts`), not the full suite (deliberately scoped small — see `docs/OPERATIONAL_RUNBOOK.md`'s "Production smoke gate (CI)" section). The full 151-test Playwright suite (all 43 accessibility tests included) still runs manually against a real production build before every release, unchanged. |
