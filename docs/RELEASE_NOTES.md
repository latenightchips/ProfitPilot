...
**Current release.** Promotes the Portfolio Analytics — Price &
Liquidation Trend Visibility batch below out of Unreleased status —
built, tested, and independently re-verified (4119/4119 tests passing,
both in the implementation worktree and again after applying the
delivered patch to a clean checkout) in the same batch that produced
it, not a fresh Milestone-9/V1.1-style Release Candidate process with
its own new manual exploratory pass. See `PROJECT_STATUS.md`'s "v1.5.0
Release Reconciliation" section and `docs/CHANGELOG.md`'s `[1.5.0]`
entry for the full record. Everything in "Version 1.4.0" and earlier
below still applies; this section covers only what is new since 1.4.0.
