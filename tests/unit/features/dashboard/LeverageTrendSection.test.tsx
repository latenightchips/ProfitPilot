...
import { LeverageTrendSection } from '@/features/dashboard/components/LeverageTrendSection';
...
/**
 * `LeverageTrendSection` — v1.10.0 Batch 2 ("Dashboard Trend Parity").
 ...
 *
 * `leverage` is always a required, non-null number for every entry
 * regardless of protocol version, so there is no "no risk"/"not
...
describe('LeverageTrendSection — empty state', () => {
  it('explains there is no history yet, rather than rendering an empty chart', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('No Leverage history yet.')).toBeInTheDocument();
    });
    expect(screen.queryByRole('img')).not.toBeInTheDocument();
    expect(screen.getByText('Leverage Trend')).toBeInTheDocument();
  });
});

describe('LeverageTrendSection — error state', () => {
...
describe('LeverageTrendSection — single usable point', () => {
  it('shows the single value as text rather than a one-point chart, never fabricating a second point', async () => {
    await recordPortfolioHistoryEntry(entry({ leverage: 1.25 }));
    render(
      <LeverageTrendSection
        portfolioId="portfolio-1"
        portfolioUpdatedAt="2026-01-01T00:00:00.000Z"
  ...
  it('renders exactly 1x for an unleveraged portfolio, never a fabricated value', async () => {
    await recordPortfolioHistoryEntry(entry({ leverage: 1 }));
    render(
      <LeverageTrendSection
        portfolioId="portfolio-1"
        portfolioUpdatedAt="2026-01-01T00:00:00.000Z"
    ...
    expect(screen.getByText(/\b1x/)).toBeInTheDocument();
  });
});

describe('LeverageTrendSection — multiple entries', () => {
  it('renders an accessible chart with 2+ entries, clearly identified as a Leverage trend', async () => {
    ...
    expect(chart.getAttribute('aria-label')).toContain('Leverage trend');
    expect(screen.getByText('Leverage Trend')).toBeInTheDocument();
  });
  ...
  it('preserves chronological ordering (oldest first) in the accessible summary, regardless of the service’s newest-first read order', async () => {
    ...
    await recordPortfolioHistoryEntry(
      entry({ createdAt: '2026-02-01T00:00:00.000Z', leverage: 2 }),
    );
    render(
      <LeverageTrendSection
        portfolioId="portfolio-1"
    ...
    const oldestPoint = `${dateTimeFormatter.format(new Date('2026-01-01T00:00:00.000Z'))} 1.25x`;
    const newestPoint = `${dateTimeFormatter.format(new Date('2026-02-01T00:00:00.000Z'))} 2x`;
  ...
  it('keeps multiple portfolios isolated — only the requested portfolioId’s entries feed the chart', async () => {
    ...
    expect(label).toContain('1.25x');
    expect(label).toContain('1.5x');
    expect(label).not.toContain('3x');
    expect(label).not.toContain('3.5x');
  });

  it('works for a V3 portfolio entry (explicit protocolVersion "v3", the default in this codebase)', async () => {
    await recordPortfolioHistoryEntry(
      entry({ createdAt: '2026-01-01T00:00:00.000Z', protocolVersion: 'v3', leverage: 1.25 }),
    );
    await recordPortfolioHistoryEntry(
      entry({ createdAt: '2026-02-01T00:00:00.000Z', protocolVersion: 'v3', leverage: 1.5 }),
    );
    render(
      <LeverageTrendSection
        portfolioId="portfolio-1"
