...
import { DebtValueTrendSection } from '@/features/dashboard/components/DebtValueTrendSection';
...
/**
 * `DebtValueTrendSection` — v1.14.0 Batch 1 ("Dashboard Trend Parity,
 * Part 2"). Follows the exact same real-persistence-seeding pattern
 * `tests/unit/features/dashboard/NetWorthTrendSection.test.tsx`
 ...
 *
 * `debt.valueUsd` is an always-required, non-null number for every
 * entry regardless of protocol version (zero debt is `0`, not `null` or
 * `undefined`), so there is no "no risk"/"not available" branch to
 * cover here — only a plain, directly-read numeric value.
 */
function entry(
...
describe('DebtValueTrendSection — empty state', () => {
  it('explains there is no history yet, rather than rendering an empty chart', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('No Debt Value history yet.')).toBeInTheDocument();
    });
    expect(screen.queryByRole('img')).not.toBeInTheDocument();
    expect(screen.getByText('Debt Value Trend')).toBeInTheDocument();
  });
});

describe('DebtValueTrendSection — error state', () => {
...
describe('DebtValueTrendSection — single usable point', () => {
  ...
  it('renders a zero debt value without special-casing (never treated as null/unavailable)', async () => {
    await recordPortfolioHistoryEntry(
      entry({
...
describe('DebtValueTrendSection — multiple entries', () => {
  it('renders an accessible chart with 2+ entries, clearly identified as a Debt Value trend', async () => {
    await recordPortfolioHistoryEntry(
      entry({
        createdAt: '2026-01-01T00:00:00.000Z',
        debt: { asset: 'USDC', quantity: 20000, valueUsd: 20000 },
    ...
    expect(chart.getAttribute('aria-label')).toContain('Debt Value trend');
    expect(screen.getByText('Debt Value Trend')).toBeInTheDocument();
  });
  ...
  it('preserves chronological ordering (oldest first) in the accessible summary, regardless of the service’s newest-first read order', async () => {
    ...
    const oldestPoint = `${dateTimeFormatter.format(new Date('2026-01-01T00:00:00.000Z'))} $20,000.00`;
    const newestPoint = `${dateTimeFormatter.format(new Date('2026-02-01T00:00:00.000Z'))} $30,000.00`;
  ...
  it('keeps multiple portfolios isolated — only the requested portfolioId’s entries feed the chart', async () => {
    ...
    expect(label).toContain('$20,000.00');
    expect(label).toContain('$25,000.00');
    expect(label).not.toContain('$50,000.00');
    expect(label).not.toContain('$60,000.00');
  });
  ...
  it('renders a debt-asset change between snapshots without needing the asset symbol (currency-only, not quantity)', async () => {
    await recordPortfolioHistoryEntry(
      entry({
        createdAt: '2026-01-01T00:00:00.000Z',
        debt: { asset: 'USDC', quantity: 20000, valueUsd: 20000 },
    ...
    await recordPortfolioHistoryEntry(
      entry({
        createdAt: '2026-02-01T00:00:00.000Z',
        debt: { asset: 'DAI', quantity: 25000, valueUsd: 25000 },
      }),
    );
    render(
      <DebtValueTrendSection
