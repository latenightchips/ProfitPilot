...
import { CollateralValueTrendSection } from '@/features/dashboard/components/CollateralValueTrendSection';
...
/**
 * `CollateralValueTrendSection` — v1.14.0 Batch 1 ("Dashboard Trend
 * Parity, Part 2"). Follows the exact same real-persistence-seeding
 * pattern `tests/unit/features/dashboard/NetWorthTrendSection.test.tsx`
 ...
 *
 * `collateral.valueUsd` is an always-required, non-null number for
 * every entry regardless of protocol version, so there is no "no
 * risk"/"not available" branch to cover here — only a plain,
 * directly-read numeric value.
 */
function entry(
...
describe('CollateralValueTrendSection — empty state', () => {
  it('explains there is no history yet, rather than rendering an empty chart', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('No Collateral Value history yet.')).toBeInTheDocument();
    });
    expect(screen.queryByRole('img')).not.toBeInTheDocument();
    expect(screen.getByText('Collateral Value Trend')).toBeInTheDocument();
  });
});

describe('CollateralValueTrendSection — error state', () => {
...
describe('CollateralValueTrendSection — single usable point', () => {
...
describe('CollateralValueTrendSection — multiple entries', () => {
  it('renders an accessible chart with 2+ entries, clearly identified as a Collateral Value trend', async () => {
    await recordPortfolioHistoryEntry(
      entry({
        createdAt: '2026-01-01T00:00:00.000Z',
        collateral: { quantity: 2, valueUsd: 100000 },
    ...
    expect(chart.getAttribute('aria-label')).toContain('Collateral Value trend');
    expect(screen.getByText('Collateral Value Trend')).toBeInTheDocument();
  });
  ...
  it('states the exact displayed/accessible value for every plotted point in the aria-label summary', async () => {
    ...
    expect(label).toContain('$120,000.00');
  });
  ...
  it('preserves chronological ordering (oldest first) in the accessible summary, regardless of the service’s newest-first read order', async () => {
    ...
    const oldestPoint = `${dateTimeFormatter.format(new Date('2026-01-01T00:00:00.000Z'))} $100,000.00`;
    const newestPoint = `${dateTimeFormatter.format(new Date('2026-02-01T00:00:00.000Z'))} $150,000.00`;
  ...
  it('keeps multiple portfolios isolated — only the requested portfolioId’s entries feed the chart', async () => {
    ...
    expect(label).toContain('$100,000.00');
    expect(label).toContain('$120,000.00');
    expect(label).not.toContain('$500,000.00');
    expect(label).not.toContain('$600,000.00');
  });
